...
pub fn ruby_mac_app_store_spec_l10_d1_dumper(args ...ruby.Value) ruby.Value {
	return ruby.object_value('Homebrew::Bundle::MacAppStore', 'Homebrew::Bundle::MacAppStore')
}

// Ruby specify `specify do` at line 18.
pub fn ruby_mac_app_store_spec_l18_d2_do(args ...ruby.Value) ruby.Value {
	mut state := extensions.MacAppStoreState{}
	return ruby.bool_value(extensions.mac_app_store_apps(mut state).len == 0 && extensions.mac_app_store_dump(mut state) == '')
}

// Ruby specify `specify do` at line 31.
pub fn ruby_mac_app_store_spec_l31_d3_do(args ...ruby.Value) ruby.Value {
	mut state := extensions.MacAppStoreState{ executable: 'mas' }
	return ruby.bool_value(extensions.mac_app_store_apps(mut state).len == 0 && extensions.mac_app_store_dump(mut state) == '')
}

// Ruby it `it "returns list %w[foo bar baz]" do` at line 48.
pub fn ruby_mac_app_store_spec_l48_d4_returns(args ...ruby.Value) ruby.Value {
	mut state := extensions.MacAppStoreState{ executable: 'mas', list_output: '123 foo (1.0)\n456 bar (2.0)\n789 baz (3.0)' }
	return ruby.bool_value(extensions.mac_app_store_apps(mut state) == [
...
pub fn ruby_mac_app_store_spec_l61_d5_returns(args ...ruby.Value) ruby.Value {
	mut state := extensions.MacAppStoreState{ executable: 'mas', list_output: '123 foo (1.0)\n456 bar (2.0)\n789 baz (3.0)\n 10 qux (4.0)' }
	return ruby.bool_value(extensions.mac_app_store_apps(mut state).map(it.name) == [
		'foo',
		'bar',
		'baz',
		'qux',
...
pub fn ruby_mac_app_store_spec_l67_d6_invalid_mas_output(args ...ruby.Value) ruby.Value {
	return ruby.string_value(mac_app_store_spec_invalid_output())
}

// Ruby let `let(:expected_app_details_array) do` at line 91.
pub fn ruby_mac_app_store_spec_l91_d7_expected_app_details_array(args ...ruby.Value) ruby.Value {
	return ruby.array_value(mac_app_store_spec_expected_apps().map(ruby.string_array_value([
		it.id,
		it.name,
	])))
}
...
pub fn ruby_mac_app_store_spec_l115_d8_expected_mas_dumped_output(args ...ruby.Value) ruby.Value {
	mut state := extensions.MacAppStoreState{ executable: 'mas', list_output: mac_app_store_spec_invalid_output() }
	return ruby.string_value(extensions.mac_app_store_dump(mut state))
}

// Ruby specify `specify do` at line 145.
pub fn ruby_mac_app_store_spec_l145_d9_do(args ...ruby.Value) ruby.Value {
	mut state := extensions.MacAppStoreState{ executable: 'mas', list_output: mac_app_store_spec_invalid_output() }
	apps := extensions.mac_app_store_apps(mut state)
	dump_text := extensions.mac_app_store_dump(mut state)
	return ruby.bool_value(apps == mac_app_store_spec_expected_apps() && dump_text == mac_app_store_spec_expected_dump())
}

// Ruby let `let(:new_mas_output) do` at line 152.
pub fn ruby_mac_app_store_spec_l152_d10_new_mas_output(args ...ruby.Value) ruby.Value {
	return ruby.string_value(mac_app_store_spec_new_output())
}

// Ruby let `let(:expected_app_details_array) do` at line 160.
pub fn ruby_mac_app_store_spec_l160_d11_expected_app_details_array(args ...ruby.Value) ruby.Value {
	return ruby.array_value(mac_app_store_spec_new_apps().map(ruby.string_array_value([
		it.id,
		it.name,
	])))
}
...
pub fn ruby_mac_app_store_spec_l174_d12_parses(args ...ruby.Value) ruby.Value {
	return ruby.bool_value(extensions.mac_app_store_parse_apps(mac_app_store_spec_new_output()) == mac_app_store_spec_new_apps())
}

// Ruby it `it "shells out" do` at line 189.
pub fn ruby_mac_app_store_spec_l189_d13_shells(args ...ruby.Value) ruby.Value {
	mut state := extensions.MacAppStoreState{ executable: 'mas', list_output: '123 foo (1.0)' }
	return ruby.bool_value(extensions.mac_app_store_installed_app_ids(mut state) == [
		'123',
	])
}

// Ruby it `it "returns result" do` at line 195.
pub fn ruby_mac_app_store_spec_l195_d14_returns(args ...ruby.Value) ruby.Value {
	mut state := extensions.MacAppStoreState{
		executable: 'mas'
		installed_app_ids: [
			'123',
	...
	return ruby.bool_value(extensions.mac_app_store_app_id_installed_and_up_to_date(mut state, 123, false) && !extensions.mac_app_store_app_id_installed_and_up_to_date(mut state, 456, false))
}

// Ruby it `it "tries to install mas" do` at line 208.
pub fn ruby_mac_app_store_spec_l208_d15_tries(args ...ruby.Value) ruby.Value {
	...
	_ := extensions.mac_app_store_preinstall(mut state, 'foo', 123, false, false) or {
		return ruby.bool_value(state.commands == [['/brew', 'install', 'mas']] && err.msg().contains('mas installation failed'))
	}
	return ruby.bool_value(false)
}

// Ruby it `it "does not shell out" do` at line 215.
pub fn ruby_mac_app_store_spec_l215_d16_does(args ...ruby.Value) ruby.Value {
	mut state := extensions.MacAppStoreState{}
	return ruby.bool_value(extensions.mac_app_store_outdated_app_ids(mut state).len == 0)
}

// Ruby it `it "returns app ids" do` at line 229.
pub fn ruby_mac_app_store_spec_l229_d17_returns(args ...ruby.Value) ruby.Value {
	mut state := extensions.MacAppStoreState{ executable: 'mas', outdated_output: 'foo 123' }
	return ruby.bool_value(extensions.mac_app_store_outdated_app_ids(mut state) == [
		'foo',
	])
}

// Ruby it `it "skips" do` at line 241.
pub fn ruby_mac_app_store_spec_l241_d18_skips(args ...ruby.Value) ruby.Value {
	mut state := extensions.MacAppStoreState{
		executable: 'mas'
		installed_app_ids: [
			'123',
	...
	result := extensions.mac_app_store_preinstall(mut state, 'foo', 123, false, false) or { return ruby.bool_value(false) }
	return ruby.bool_value(!result && state.commands.len == 0)
}

// Ruby it `it "upgrades" do` at line 252.
pub fn ruby_mac_app_store_spec_l252_d19_upgrades(args ...ruby.Value) ruby.Value {
	mut state := extensions.MacAppStoreState{
		executable: 'mas'
		installed_app_ids: [
			'123',
	...
	preinstall := extensions.mac_app_store_preinstall(mut state, 'foo', 123, false, false) or { return ruby.bool_value(false) }
	installed := extensions.mac_app_store_install(mut state, 'foo', 123, true, false) or { return ruby.bool_value(false) }
	return ruby.bool_value(preinstall && installed && state.commands == [
		['mas', 'upgrade', '123'],
	])
}

// Ruby it `it "installs app" do` at line 265.
pub fn ruby_mac_app_store_spec_l265_d20_installs(args ...ruby.Value) ruby.Value {
...
pub fn ruby_mac_app_store_spec_l272_d21_falls(args ...ruby.Value) ruby.Value {
...
pub fn ruby_mac_app_store_spec_l294_d22_returns(args ...ruby.Value) ruby.Value {
	mut state := mac_app_store_spec_cleanup_state()
	entry := extensions.mac_app_store_entry('renamed foo', {
		'id': ruby.int_value(123)
	}) or { return ruby.bool_value(false) }
	items := extensions.mac_app_store_cleanup_items(mut state, [entry])
	if items.len != 1 {
		return ruby.bool_value(false)
	}
	name := extensions.mac_app_store_cleanup_item_name(items[0]) or { return ruby.bool_value(false) }
	return ruby.bool_value(name == 'bar (456)')
}

// Ruby it `it "uninstalls apps by ID" do` at line 301.
pub fn ruby_mac_app_store_spec_l301_d23_uninstalls(args ...ruby.Value) ruby.Value {
	mut state := mac_app_store_spec_cleanup_state()
	entry := extensions.mac_app_store_entry('foo', {
		'id': ruby.int_value(123)
	}) or { return ruby.bool_value(false) }
	items := extensions.mac_app_store_cleanup_items(mut state, [entry])
	extensions.mac_app_store_cleanup(mut state, items) or { return ruby.bool_value(false) }
	return ruby.bool_value(state.commands == [['mas', 'uninstall', '456']] && state.output == [
