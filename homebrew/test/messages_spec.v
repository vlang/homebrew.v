...
pub fn ruby_messages_spec_l8_d1_messages(args ...ruby.Value) ruby.Value {
	messages := homebrew.new_messages()
	return ruby.structured_value('Messages', 'Messages', {
...
pub fn ruby_messages_spec_l9_d2_test_formula(args ...ruby.Value) ruby.Value {
	return ruby.structured_value('Formula', 'foo', {
		'name': 'foo'
...
pub fn ruby_messages_spec_l15_d3_elapsed_time(args ...ruby.Value) ruby.Value {
	return ruby.float_value(1.1)
}

// Ruby it `it "adds a caveat" do` at line 18.
pub fn ruby_messages_spec_l18_d4_adds(args ...ruby.Value) ruby.Value {
	mut messages := homebrew.new_messages()
	before := messages.caveats.len
	messages.record_caveats('foo', 'Zsh completions were installed')
	return ruby.bool_value(messages.caveats.len == before + 1)
}

// Ruby it `it "increases the package count" do` at line 26.
pub fn ruby_messages_spec_l26_d5_increases(args ...ruby.Value) ruby.Value {
	mut messages := homebrew.new_messages()
	before := messages.package_count
	messages.package_installed('foo', 1.1)
	return ruby.bool_value(messages.package_count == before + 1)
}

// Ruby it `it "adds to install_times" do` at line 32.
pub fn ruby_messages_spec_l32_d6_adds(args ...ruby.Value) ruby.Value {
	mut messages := homebrew.new_messages()
	before := messages.install_times.len
	messages.package_installed('foo', 1.1)
	return ruby.bool_value(messages.install_times.len == before + 1)
}

// Ruby it `it "doesn't print caveat details" do` at line 46.
pub fn ruby_messages_spec_l46_d7_doesn(args ...ruby.Value) ruby.Value {
	mut messages := homebrew.new_messages()
	messages.record_caveats('foo', 'Zsh completions were installed')
	messages.package_installed('foo', 1.1)
	return ruby.bool_value(messages.display_messages(false, false) == '')
}

// Ruby it `it "doesn't print caveat details" do` at line 56.
pub fn ruby_messages_spec_l56_d8_doesn(args ...ruby.Value) ruby.Value {
	mut messages := homebrew.new_messages()
	messages.package_installed('foo', 1.1)
	return ruby.bool_value(messages.display_messages(false, false) == '')
}
...
pub fn ruby_messages_spec_l62_d9_test_formula2(args ...ruby.Value) ruby.Value {
	return ruby.structured_value('Formula', 'bar', {
		'name': 'bar'
...
pub fn ruby_messages_spec_l75_d10_prints(args ...ruby.Value) ruby.Value {
	mut messages := homebrew.new_messages()
	messages.record_caveats('foo', 'Zsh completions were installed')
	messages.package_installed('foo', 1.1)
	messages.package_installed('bar', 1.1)
	return ruby.bool_value(messages.display_messages(false, false) == '==> Caveats\n==> foo\nZsh completions were installed')
}

// Ruby it `it "doesn't print anything" do` at line 88.
pub fn ruby_messages_spec_l88_d11_doesn(args ...ruby.Value) ruby.Value {
	messages := homebrew.new_messages()
	return ruby.bool_value(messages.display_messages(false, true) == '')
}

// Ruby it `it "prints installation times" do` at line 98.
pub fn ruby_messages_spec_l98_d12_prints(args ...ruby.Value) ruby.Value {
	mut messages := homebrew.new_messages()
	messages.package_installed('foo', 1.1)
	return ruby.bool_value(messages.display_messages(false, true) == '==> Installation times\nfoo                       1.100 s')
}

// Ruby it `it "doesn't print installation times" do` at line 110.
pub fn ruby_messages_spec_l110_d13_doesn(args ...ruby.Value) ruby.Value {
	messages := homebrew.new_messages()
	return ruby.bool_value(messages.display_messages(false, false) == '')
}
