...
fn bash_completion_spec_cask_value(cask BashCompletionSpecCask) ruby.Value {
	return ruby.Value{
		type_name: 'Cask::Cask'
		repr: cask.token
		map_data: {
			...
			'bash_completion_directory': ruby.string_value(cask.bash_completion_directory)
			'completion_source':         ruby.string_value(cask.completion_source)
		}
	}
}

fn bash_completion_spec_cask_from_value(value ruby.Value) !BashCompletionSpecCask {
...
pub fn ruby_bashcompletion_spec_l5_d1_cask_token(args ...ruby.Value) ruby.Value {
	_ = args
	return ruby.string_value('basic-cask')
}

// Ruby let `let(:cask) { Cask::CaskLoader.load(cask_token) }` at line 6.
pub fn ruby_bashcompletion_spec_l6_d2_cask(args ...ruby.Value) ruby.Value {
...
pub fn ruby_bashcompletion_spec_l9_d3_install_phase(args ...ruby.Value) ruby.Value {
	if args.len == 0 {
		return ruby.object_value('Proc', 'install BashCompletion artifacts without sudo or force')
	}
...
pub fn ruby_bashcompletion_spec_l17_d4_source_path(args ...ruby.Value) ruby.Value {
	...
	return ruby.object_value('Pathname', os.join_path(cask.staged_path, 'test.bash'))
}

// Ruby let `let(:target_path) { cask.config.bash_completion.join("test") }` at line 18.
pub fn ruby_bashcompletion_spec_l18_d5_target_path(args ...ruby.Value) ruby.Value {
	cask_value := if args.len > 0 { args[0] } else { ruby_bashcompletion_spec_l6_d2_cask() }
	cask := bash_completion_spec_cask_from_value(cask_value) or {
		return ruby.object_value('ArgumentError', err.msg())
	}
	return ruby.object_value('Pathname', os.join_path(cask.bash_completion_directory, 'test'))
}

// Ruby let `let(:full_source_path) { cask.staged_path.join("test.bash-completion") }` at line 19.
pub fn ruby_bashcompletion_spec_l19_d6_full_source_path(args ...ruby.Value) ruby.Value {
	cask_value := if args.len > 0 {
		args[0]
	} else {
		ruby_bashcompletion_spec_l6_d2_cask(ruby.string_value('with-shellcompletion-long'))
	}
	cask := bash_completion_spec_cask_from_value(cask_value) or {
		return ruby.object_value('ArgumentError', err.msg())
	}
	return ruby.object_value('Pathname', os.join_path(cask.staged_path, 'test.bash-completion'))
}

// Ruby let `let(:full_target_path) { cask.config.bash_completion.join("test") }` at line 20.
pub fn ruby_bashcompletion_spec_l20_d7_full_target_path(args ...ruby.Value) ruby.Value {
	cask_value := if args.len > 0 {
		args[0]
	} else {
		ruby_bashcompletion_spec_l6_d2_cask(ruby.string_value('with-shellcompletion-long'))
...
pub fn ruby_bashcompletion_spec_l23_d8_cask_token(args ...ruby.Value) ruby.Value {
	_ = args
	return ruby.string_value('with-shellcompletion')
}

// Ruby it `it "links the completion to the proper directory" do` at line 25.
pub fn ruby_bashcompletion_spec_l25_d9_links(args ...ruby.Value) ruby.Value {
	_ = args
	return ruby.bool_value(bash_completion_spec_links('with-shellcompletion'))
}

// Ruby let `let(:cask_token) { "with-shellcompletion-long" }` at line 36.
pub fn ruby_bashcompletion_spec_l36_d10_cask_token(args ...ruby.Value) ruby.Value {
	_ = args
	return ruby.string_value('with-shellcompletion-long')
}

// Ruby it `it "links the completion to the proper directory" do` at line 38.
pub fn ruby_bashcompletion_spec_l38_d11_links(args ...ruby.Value) ruby.Value {
	_ = args
	return ruby.bool_value(bash_completion_spec_links('with-shellcompletion-long'))
}
