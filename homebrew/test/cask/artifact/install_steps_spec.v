...
fn cask_install_steps_spec_path(path string, base string) ruby.Value {
	mut values := {
		'path': ruby.string_value(path)
	}
	if base != '' {
		values['base'] = ruby.string_value(base)
	}
	return ruby.map_value(values)
}

fn cask_install_steps_spec_step(kind string,
	fields map[string]ruby.Value) homebrew.InstallStep {
	mut step := fields.clone()
	step['type'] = ruby.string_value(kind)
	return homebrew.InstallStep(step)
}

fn cask_install_steps_spec_steps(cask ruby.Value,
...
fn cask_install_steps_spec_root(args []ruby.Value, suffix string) string {
...
fn cask_install_steps_spec_cask(root string) ruby.Value {
	...
	preflight_steps := [cask_install_steps_spec_step('mkdir_p', {
		...
		'paths':       ruby.array_value([
			cask_install_steps_spec_path('Prepared', 'staged_path'),
		])
		'permissions': ruby.string_value('0755')
	...
	return ruby.Value{
		...
		map_data: {
			'name':                       ruby.string_value('with-install-steps')
			'token':                      ruby.string_value('with-install-steps')
			'version':                    ruby.string_value('1.2.3')
			'staged_path':                ruby.string_value(staged_path)
			'caskroom_path':              ruby.string_value(os.join_path(root, 'Caskroom', 'with-install-steps'))
...
fn cask_install_steps_spec_artifact(cask ruby.Value, key string,
...
pub fn ruby_install_steps_spec_l9_d1_cask(args ...ruby.Value) ruby.Value {
...
pub fn ruby_install_steps_spec_l38_d2_runs(args ...ruby.Value) ruby.Value {
	...
	os.mkdir_all(staged_path) or { return ruby.bool_value(false) }
	...
	mode := os.stat(prepared) or { return ruby.bool_value(false) }
...
pub fn ruby_install_steps_spec_l63_d3_omits(args ...ruby.Value) ruby.Value {
	...
	for step in steps {
		if (step['type'] or { continue }).as_string() == 'run' {
			return ruby.bool_value('print_stdout' !in step && 'suppress_stderr' !in step && 'writable_paths' !in step && 'network_access' !in step)
		}
	}
	return ruby.bool_value(false)
}
...
pub fn ruby_install_steps_spec_l70_d4_sandboxes(args ...ruby.Value) ruby.Value {
	...
	steps := [cask_install_steps_spec_step('touch', {
		...
		'writable_paths': ruby.array_value([
	...
	return ruby.bool_value(!plan.network_access_allowed
...
pub fn ruby_install_steps_spec_l151_d6_allows(args ...ruby.Value) ruby.Value {
	...
	steps := [
		...
		cask_install_steps_spec_step('delete_keychain_certificate', {
			'name': ruby.string_value('Example')
		}),
	]
	for step in steps {
		artifact := install_steps_artifact.new_cask_install_steps_artifact(cask, [
...
pub fn ruby_install_steps_spec_l174_d7_runs(args ...ruby.Value) ruby.Value {
	...
	install_steps_artifact.run_cask_install_steps(artifact, 'install', false, CaskInstallStepsSpecExecutor{}) or { return ruby.bool_value(false) }
	steps_ran := os.join_path(staged_path, 'steps-ran')
	if !os.exists(steps_ran) {
		return ruby.bool_value(false)
	}
	ruby_block_ran := os.join_path(staged_path, 'ruby-block-ran')
	os.write_file(ruby_block_ran, '') or { return ruby.bool_value(false) }
	return ruby.bool_value(os.exists(steps_ran) && os.exists(ruby_block_ran))
}
