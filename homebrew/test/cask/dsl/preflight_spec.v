...
pub fn ruby_preflight_spec_l8_d1_cask(args ...ruby.Value) ruby.Value {
...
pub fn ruby_preflight_spec_l9_d2_fake_system_command(args ...ruby.Value) ruby.Value {
	return ruby.object_value('Class', 'SystemCommand')
}

// Ruby let `let(:dsl) { described_class.new(cask, fake_system_command) }` at line 10.
pub fn ruby_preflight_spec_l10_d3_dsl(args ...ruby.Value) ruby.Value {
	cask := if args.len > 0 { args[0] } else { ruby_preflight_spec_l8_d1_cask() }
	command := if args.len > 1 { args[1] } else { ruby_preflight_spec_l9_d2_fake_system_command() }
	return ruby.structured_value('Cask::DSL::Preflight', cask.as_string(), {
		'cask':           cask.as_string()
...
pub fn ruby_preflight_spec_l15_d4_staged(args ...ruby.Value) ruby.Value {
	return if args.len > 0 { args[0] } else { ruby_preflight_spec_l10_d3_dsl() }
}
