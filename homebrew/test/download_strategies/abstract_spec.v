...
pub fn ruby_abstract_spec_l7_d1_strategy(args ...brew_runtime.Value) brew_runtime.Value {
	...
	return brew_runtime.structured_value('AbstractDownloadStrategy', strategy.url, {
		'url':     strategy.url
		'name':    strategy.name
...
pub fn ruby_abstract_spec_l14_d6_source_modified_time(args ...brew_runtime.Value) brew_runtime.Value {
	...
	if owned {
		...
		os.write_file(os.join_path(root, 'foo'), '') or { return brew_runtime.bool_value(false) }
		os.write_file(os.join_path(root, 'bar'), '') or { return brew_runtime.bool_value(false) }
		os.utime(os.join_path(root, 'foo'), 90, 90) or { return brew_runtime.bool_value(false) }
		os.utime(os.join_path(root, 'bar'), 10, 10) or { return brew_runtime.bool_value(false) }
	...
	return brew_runtime.bool_value(modified == expected)
}
