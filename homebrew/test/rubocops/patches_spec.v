...
const patches_spec_sha = '63376b8fdd6613a91976106d9376069274191860cd58f039b29ff16de1925621'

fn patches_spec_formula(body string) string {
	indented := body.split('\n').map('  ${it}').join('\n')
	return 'class Foo < Formula\n  url "https://brew.sh/foo-1.0.tgz"\n${indented}\nend'
}

fn patches_spec_external(url string, sha bool) string {
	mut body := 'patch do\n  url "${url}"'
	if sha {
		body += '\n  sha256 "${patches_spec_sha}"'
	}
	body += '\nend'
	return patches_spec_formula(body)
}

fn patches_spec_external_metadata(url string, statement string) string {
	return patches_spec_formula('patch do\n  url "${url}"\n  sha256 "${patches_spec_sha}"\n  ${statement}\nend')
}

fn patches_spec_analysis(source string) patch_cop.FormulaPatchesAnalysis {
	return patch_cop.audit_formula_patches(patch_cop.FormulaPatchesContext{
		source: source
	})
}

fn patches_spec_messages(source string) []string {
	return patches_spec_analysis(source).offenses.map(it.message)
}

fn patches_spec_external_message(url string, message string) bool {
	return patches_spec_messages(patches_spec_external(url, true)) == [message]
}

fn patches_spec_corrects(url string, corrected_url string, reset_sha bool, message string) bool {
	source := patches_spec_external(url, true)
	analysis := patches_spec_analysis(source)
	mut expected := patches_spec_external(corrected_url, true)
	if reset_sha {
		expected = expected.replace('sha256 "${patches_spec_sha}"', 'sha256 ""')
	}
	return analysis.offenses.len == 1 && analysis.offenses[0].message == message && analysis.corrected == expected
}
...
pub fn ruby_patches_spec_l35_d6_reports(args ...ruby.Value) ruby.Value {
	urls := {
		...
		'https://patch-diff.githubusercontent.com/raw/foo/foo-bar/pull/100.patch':                           'Use a commit hash URL rather than patch-diff: https://patch-diff.githubusercontent.com/raw/foo/foo-bar/pull/100.patch'
		'https://github.com/dlang/dub/commit/2c916b1a7999a050ac4970c3415ff8f91cd487aa.patch':                'GitHub patches should use the full_index parameter: https://github.com/dlang/dub/commit/2c916b1a7999a050ac4970c3415ff8f91cd487aa.patch?full_index=1'
		'https://bitbucket.org/multicoreware/x265_git/commits/b354c009a60bcd6d7fc04014e200a1ee9c45c167/raw': 'Bitbucket patches should use the API URL: https://api.bitbucket.org/2.0/repositories/multicoreware/x265_git/diff/b354c009a60bcd6d7fc04014e200a1ee9c45c167'
	}
...
pub fn ruby_patches_spec_l177_d14_reports(args ...ruby.Value) ruby.Value {
	urls := {
		...
		'https://mirrors.ustc.edu.cn/macports/trunk/':                             'MacPorts patches should specify a revision instead of trunk: https://mirrors.ustc.edu.cn/macports/trunk/'
		'https://patch-diff.githubusercontent.com/raw/foo/foo-bar/pull/100.patch': 'Use a commit hash URL rather than patch-diff: https://patch-diff.githubusercontent.com/raw/foo/foo-bar/pull/100.patch'
		'https://github.com/uber/h3/pull/362.patch?full_index=1':                  'Use a commit hash URL rather than an unstable pull request URL: https://github.com/uber/h3/pull/362.patch?full_index=1'
		'https://gitlab.gnome.org/GNOME/gitg/-/merge_requests/142.diff':           'Use a commit hash URL rather than an unstable merge request URL: https://gitlab.gnome.org/GNOME/gitg/-/merge_requests/142.diff'
	}
	for url, message in urls {
