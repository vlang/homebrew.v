...
fn os_depends_on_spec_source(lines []string) string {
	return lines.join('\n') + '\n'
}

fn os_depends_on_spec_matches(source string, messages []string, snippets []string, corrected string,
	correctable []bool) bool {
	analysis := os_depends_on_cop.analyze_os_depends_on(source)
	if analysis.offenses.len != messages.len || snippets.len != messages.len || correctable.len != messages.len || analysis.corrected != corrected {
		return false
	}
	for index, offense in analysis.offenses {
		if offense.message != messages[index] || offense.has_correction != correctable[index] || source[offense.begin_pos..offense.end_pos] != snippets[index] {
			return false
		}
	}
	return true
...
fn os_depends_on_spec_accepts(source string) bool {
	analysis := os_depends_on_cop.analyze_os_depends_on(source)
...
pub fn ruby_os_depends_on_spec_l7_d1_autocorrects(args ...brew_runtime.Value) brew_runtime.Value {
	...
	corrected := os_depends_on_spec_source([
		'depends_on macos: :catalina',
	...
	], ['">= :catalina"', '"<= :sonoma"', '"<= :tahoe"'], corrected, [true, true, true]))
}
