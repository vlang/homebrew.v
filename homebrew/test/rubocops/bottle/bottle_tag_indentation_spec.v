...
pub fn ruby_bottle_tag_indentation_spec_l9_d2_reports() bool {
	return bottle_core.audit_bottle_tag_indentation(bottle_spec_formula('  bottle :unneeded')).len == 0
}

// Ruby it `it "reports no offenses for `bottle` blocks without cellars" do` at line 19.
pub fn ruby_bottle_tag_indentation_spec_l19_d3_reports() bool {
	...
	return bottle_core.audit_bottle_tag_indentation(multiple).len == 0 && bottle_core.audit_bottle_tag_indentation(single).len == 0
}

// Ruby it `it "reports no offenses for properly aligned tags in `bottle` blocks with cellars" do` at line 44.
pub fn ruby_bottle_tag_indentation_spec_l44_d4_reports() bool {
...
pub fn ruby_bottle_tag_indentation_spec_l69_d5_reports() bool {
	source := bottle_spec_formula('  bottle do\n    rebuild 4\n    sha256 cellar: :any, arm64_big_sur: "aaaaaaaa"\n    sha256 cellar: "/usr/local/Cellar", big_sur: "faceb00c"\n    sha256 catalina: "deadbeef"\n  end')
	expected := bottle_spec_formula('  bottle do\n    rebuild 4\n    sha256 cellar: :any,                arm64_big_sur: "aaaaaaaa"\n    sha256 cellar: "/usr/local/Cellar", big_sur: "faceb00c"\n    sha256                              catalina: "deadbeef"\n  end')
	problems := bottle_core.audit_bottle_tag_indentation(source)
	return problems.len == 2 && problems.all(it.message == bottle_core.bottle_tag_indentation_message) && source[problems[0].begin_pos..problems[0].end_pos] == 'arm64_big_sur: "aaaaaaaa"' && source[problems[1].begin_pos..problems[1].end_pos] == 'catalina: "deadbeef"' && bottle_core.correct_bottle_tag_indentation(source) == expected
}
