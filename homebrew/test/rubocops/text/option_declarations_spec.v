...
pub fn ruby_option_declarations_spec_l7_d1_cop(args ...ruby.Value) ruby.Value {
	return ruby.object_value('RuboCop::Cop::FormulaAudit::OptionDeclarations', 'OptionDeclarations')
}

// Ruby it `it "reports an offense when `build.without?` is used in homebrew/core" do` at line 10.
pub fn ruby_option_declarations_spec_l10_d2_reports(args ...ruby.Value) ruby.Value {
	source := option_declarations_spec_formula(option_declarations_spec_method('install', 'build.without? "bar"'))
	return ruby.bool_value(option_declarations_spec_reports(source, 'homebrew-core', 'Formulae in homebrew/core should not use `build.without?`.'))
}

// Ruby method `install` at line 15.
pub fn ruby_option_declarations_spec_l15_d3_install(args ...ruby.Value) ruby.Value {
	return ruby.string_value(option_declarations_spec_method('install', 'build.without? "bar"'))
}

// Ruby it `it "reports an offense when `build.with?` is used in homebrew/core" do` at line 23.
pub fn ruby_option_declarations_spec_l23_d4_reports(args ...ruby.Value) ruby.Value {
	source := option_declarations_spec_formula(option_declarations_spec_method('install', 'build.with? "bar"'))
	return ruby.bool_value(option_declarations_spec_reports(source, 'homebrew-core', 'Formulae in homebrew/core should not use `build.with?`.'))
}

// Ruby method `install` at line 28.
pub fn ruby_option_declarations_spec_l28_d5_install(args ...ruby.Value) ruby.Value {
	return ruby.string_value(option_declarations_spec_method('install', 'build.with? "bar"'))
}

// Ruby it `it "reports an offense when `build.without?` is used for a conditional dependency" do` at line 36.
pub fn ruby_option_declarations_spec_l36_d6_reports(args ...ruby.Value) ruby.Value {
	source := 'class Foo < Formula\n  depends_on "bar" if build.without?("baz")\nend'
	return ruby.bool_value(option_declarations_spec_reports(source, '', 'Use `:optional` or `:recommended` instead of `if build.without?("baz")`'))
}

// Ruby it `it "reports an offense when `build.with?` is used for a conditional dependency" do` at line 45.
pub fn ruby_option_declarations_spec_l45_d7_reports(args ...ruby.Value) ruby.Value {
	source := 'class Foo < Formula\n  depends_on "bar" if build.with?("baz")\nend'
	return ruby.bool_value(option_declarations_spec_reports(source, '', 'Use `:optional` or `:recommended` instead of `if build.with?("baz")`'))
}

// Ruby it `it "reports an offense when `build.without?` is used with `unless`" do` at line 54.
pub fn ruby_option_declarations_spec_l54_d8_reports(args ...ruby.Value) ruby.Value {
	source := option_declarations_spec_formula(option_declarations_spec_method('post_install', 'return unless build.without? "bar"'))
	return ruby.bool_value(option_declarations_spec_reports(source, '', 'Use `if build.with? "bar"` instead of `unless build.without? "bar"`'))
}

// Ruby method `post_install` at line 59.
pub fn ruby_option_declarations_spec_l59_d9_post_install(args ...ruby.Value) ruby.Value {
	return ruby.string_value(option_declarations_spec_method('post_install', 'return unless build.without? "bar"'))
}

// Ruby it `it "reports an offense when `build.with?` is used with `unless`" do` at line 67.
pub fn ruby_option_declarations_spec_l67_d10_reports(args ...ruby.Value) ruby.Value {
	source := option_declarations_spec_formula(option_declarations_spec_method('post_install', 'return unless build.with? "bar"'))
	return ruby.bool_value(option_declarations_spec_reports(source, '', 'Use `if build.without? "bar"` instead of `unless build.with? "bar"`'))
}

// Ruby method `post_install` at line 72.
pub fn ruby_option_declarations_spec_l72_d11_post_install(args ...ruby.Value) ruby.Value {
	return ruby.string_value(option_declarations_spec_method('post_install', 'return unless build.with? "bar"'))
}

// Ruby it `it "reports an offense when `build.with?` is negated" do` at line 80.
pub fn ruby_option_declarations_spec_l80_d12_reports(args ...ruby.Value) ruby.Value {
	source := option_declarations_spec_formula(option_declarations_spec_method('post_install', 'return if !build.with? "bar"'))
	return ruby.bool_value(option_declarations_spec_reports(source, '', 'Instead of negating `build.with?`, use `build.without?`'))
}

// Ruby method `post_install` at line 85.
pub fn ruby_option_declarations_spec_l85_d13_post_install(args ...ruby.Value) ruby.Value {
	return ruby.string_value(option_declarations_spec_method('post_install', 'return if !build.with? "bar"'))
}

// Ruby it `it "reports an offense when `build.without?` is negated" do` at line 93.
pub fn ruby_option_declarations_spec_l93_d14_reports(args ...ruby.Value) ruby.Value {
	source := option_declarations_spec_formula(option_declarations_spec_method('post_install', 'return if !build.without? "bar"'))
	return ruby.bool_value(option_declarations_spec_reports(source, '', 'Instead of negating `build.without?`, use `build.with?`'))
}

// Ruby method `post_install` at line 98.
pub fn ruby_option_declarations_spec_l98_d15_post_install(args ...ruby.Value) ruby.Value {
	return ruby.string_value(option_declarations_spec_method('post_install', 'return if !build.without? "bar"'))
}

// Ruby it `it "reports an offense when a `build.without?` conditional is unnecessary" do` at line 106.
pub fn ruby_option_declarations_spec_l106_d16_reports(args ...ruby.Value) ruby.Value {
	source := option_declarations_spec_formula(option_declarations_spec_method('post_install', 'return if build.without? "--without-bar"'))
	return ruby.bool_value(option_declarations_spec_reports(source, '', 'Instead of duplicating `without`, use `build.without? "bar"` to check for "--without-bar"'))
}

// Ruby method `post_install` at line 111.
pub fn ruby_option_declarations_spec_l111_d17_post_install(args ...ruby.Value) ruby.Value {
	return ruby.string_value(option_declarations_spec_method('post_install', 'return if build.without? "--without-bar"'))
}

// Ruby it `it "reports an offense when a `build.with?` conditional is unnecessary" do` at line 119.
pub fn ruby_option_declarations_spec_l119_d18_reports(args ...ruby.Value) ruby.Value {
	source := option_declarations_spec_formula(option_declarations_spec_method('post_install', 'return if build.with? "--with-bar"'))
	return ruby.bool_value(option_declarations_spec_reports(source, '', 'Instead of duplicating `with`, use `build.with? "bar"` to check for \'--with-bar\''))
}

// Ruby method `post_install` at line 124.
pub fn ruby_option_declarations_spec_l124_d19_post_install(args ...ruby.Value) ruby.Value {
	return ruby.string_value(option_declarations_spec_method('post_install', 'return if build.with? "--with-bar"'))
}

// Ruby it `it "reports an offense when `build.include?` is used" do` at line 132.
pub fn ruby_option_declarations_spec_l132_d20_reports(args ...ruby.Value) ruby.Value {
	source := option_declarations_spec_formula(option_declarations_spec_method('post_install', 'return if build.include? "foo"'))
	return ruby.bool_value(option_declarations_spec_reports(source, '', '`build.include?` is deprecated'))
}

// Ruby method `post_install` at line 137.
pub fn ruby_option_declarations_spec_l137_d21_post_install(args ...ruby.Value) ruby.Value {
	return ruby.string_value(option_declarations_spec_method('post_install', 'return if build.include? "foo"'))
}

// Ruby it `it "reports an offense when `def option` is used" do` at line 145.
pub fn ruby_option_declarations_spec_l145_d22_reports(args ...ruby.Value) ruby.Value {
	source := option_declarations_spec_formula('def options\n  [["--bar", "desc"]]\nend')
	return ruby.bool_value(option_declarations_spec_reports(source, '', 'Use new-style option definitions'))
}

// Ruby method `options` at line 151.
pub fn ruby_option_declarations_spec_l151_d23_options(args ...ruby.Value) ruby.Value {
	return ruby.string_value('def options\n  [["--bar", "desc"]]\nend')
}
