...
pub fn ruby_url_symbol_spec_l7_d1_cop(args ...brew_runtime.Value) brew_runtime.Value {
	return brew_runtime.object_value('RuboCop::Cop::FormulaAudit::LivecheckUrlSymbol', 'FormulaAudit/LivecheckUrlSymbol')
}

// Ruby it `it "reports an offense when the `url` specified in the `livecheck` block is identical to a formula URL" do` at line 9.
pub fn ruby_url_symbol_spec_l9_d2_reports(args ...brew_runtime.Value) brew_runtime.Value {
	source := 'class Foo < Formula\n  url "https://brew.sh/foo-1.0.tgz"\n\n  livecheck do\n    url "https://brew.sh/foo-1.0.tgz"\n  end\nend'
	problems := livecheck_core.audit_livecheck_url_symbol(source)
	return brew_runtime.bool_value(problems.len == 1 && problems[0].message == 'Use `url :stable`' && livecheck_core.correct_livecheck(source, problems).contains('livecheck do\n    url :stable\n'))
}
...
pub fn ruby_url_symbol_spec_l32_d3_reports(args ...brew_runtime.Value) brew_runtime.Value {
	source := 'url "https://brew.sh/foo-1.0.tgz"\nlivecheck do\n  url "https://brew.sh/foo/releases/"\nend'
	return brew_runtime.bool_value(livecheck_core.audit_livecheck_url_symbol(source).len == 0)
}
