...
pub fn ruby_skip_spec_l7_d1_cop(args ...ruby.Value) ruby.Value {
	return ruby.object_value('RuboCop::Cop::FormulaAudit::LivecheckSkip', 'FormulaAudit/LivecheckSkip')
}

// Ruby it `it "reports an offense when a skipped formula's `livecheck` block contains other information" do` at line 9.
pub fn ruby_skip_spec_l9_d2_reports(args ...ruby.Value) ruby.Value {
	source := 'livecheck do\n  skip "Not maintained"\n  url :stable\nend'
	problems := livecheck_core.audit_livecheck_skip(source)
	return ruby.bool_value(problems.len == 1 && source[problems[0].begin_pos..problems[0].end_pos] == source && livecheck_core.correct_livecheck(source, problems) == 'livecheck do\n  skip "Not maintained"\nend')
}

// Ruby it `it "reports no offenses when a skipped formula's `livecheck` block contains no other information" do` at line 33.
pub fn ruby_skip_spec_l33_d3_reports(args ...ruby.Value) ruby.Value {
	return ruby.bool_value(livecheck_core.audit_livecheck_skip('livecheck do\n  skip "Not maintained"\nend').len == 0)
}
