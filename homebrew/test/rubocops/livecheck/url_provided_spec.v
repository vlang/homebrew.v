...
pub fn ruby_url_provided_spec_l7_d1_cop(args ...brew_runtime.Value) brew_runtime.Value {
	return brew_runtime.object_value('RuboCop::Cop::FormulaAudit::LivecheckUrlProvided', 'FormulaAudit/LivecheckUrlProvided')
}

// Ruby it `it "reports an offense when a `url` is not specified in a `livecheck` block" do` at line 9.
pub fn ruby_url_provided_spec_l9_d2_reports(args ...brew_runtime.Value) brew_runtime.Value {
	regex_source := 'livecheck do\n  regex(%r{formula-(\\d+(?:\\.\\d+)+)\\.t}i)\nend'
	strategy_source := 'livecheck do\n  strategy :page_match\nend'
	return brew_runtime.bool_value(livecheck_core.audit_livecheck_url_provided(regex_source).len == 1 && livecheck_core.audit_livecheck_url_provided(strategy_source).len == 1)
}

// Ruby it `it "reports no offenses when a `url` and `regex` are specified in the `livecheck` block" do` at line 33.
pub fn ruby_url_provided_spec_l33_d3_reports(args ...brew_runtime.Value) brew_runtime.Value {
	source := 'livecheck do\n  url :stable\n  regex(%r{formula-(\\d+(?:\\.\\d+)+)\\.t}i)\nend'
	return brew_runtime.bool_value(livecheck_core.audit_livecheck_url_provided(source).len == 0)
}

// Ruby it `it "reports no offenses when a `url` and `strategy` are specified in the `livecheck` block" do` at line 46.
pub fn ruby_url_provided_spec_l46_d4_reports(args ...brew_runtime.Value) brew_runtime.Value {
	source := 'livecheck do\n  url :stable\n  strategy :page_match\nend'
	return brew_runtime.bool_value(livecheck_core.audit_livecheck_url_provided(source).len == 0)
}
