...
pub fn ruby_regex_case_insensitive_spec_l7_d1_cop(args ...ruby.Value) ruby.Value {
	return ruby.object_value('RuboCop::Cop::FormulaAudit::LivecheckRegexCaseInsensitive', 'FormulaAudit/LivecheckRegexCaseInsensitive')
}

// Ruby it `it "reports an offense when the `regex` is not case-insensitive" do` at line 9.
pub fn ruby_regex_case_insensitive_spec_l9_d2_reports(args ...ruby.Value) ruby.Value {
	...
	return ruby.bool_value(problems.len == 1 && source[problems[0].begin_pos..problems[0].end_pos] == '%r{href=.*?/formula[._-]v?(\\d+(?:\\.\\d+)+)\\.t}' && livecheck_core.correct_livecheck(source, problems).contains('\\.t}i)'))
}

// Ruby it `it "reports no offenses when the `regex` is case-insensitive" do` at line 34.
pub fn ruby_regex_case_insensitive_spec_l34_d3_reports(args ...ruby.Value) ruby.Value {
	source := 'livecheck do\n  url :stable\n  regex(%r{formula-(\\d+(?:\\.\\d+)+)\\.t}i)\nend'
	return ruby.bool_value(livecheck_core.audit_livecheck_regex_case_insensitive(source, false).len == 0)
}
