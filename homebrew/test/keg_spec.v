...
fn keg_spec_fixture(name string, version string) !KegSpecFixture {
	root := keg_spec_join(ruby.temporary_directory(), 'brew-v-keg-spec-${name}-${ruby.process_id()}')
	...
	ruby.make_dir_all(keg_spec_join(prefix, 'bin'))!
	ruby.make_dir_all(keg_spec_join(prefix, 'lib'))!
	for file in ['hiworld', 'helloworld', 'goodbye_cruel_world'] {
		ruby.write_file(keg_spec_join(bin, file), '')!
	}
...
fn keg_spec_cleanup(fixture KegSpecFixture) {
	...
	ruby.remove_all(fixture.root) or {}
}

fn keg_spec_bool(value bool) ruby.Value {
	return ruby.bool_value(value)
}

fn keg_spec_value(keg homebrew.Keg) ruby.Value {
...
fn keg_spec_symlink(target string, link string) ! {
	ruby.make_dir_all(keg_spec_parent(link))!
	result := ruby.run_captured_command(['/bin/ln', '-s', target, link], ruby.CapturedCommandOptions{})!
...
fn keg_spec_remove(path string) {
	ruby.run_captured_command(['/bin/rm', '-rf', path], ruby.CapturedCommandOptions{}) or {}
}

fn keg_spec_readlink(path string) string {
	result := ruby.run_captured_command(['/usr/bin/readlink', path], ruby.CapturedCommandOptions{}) or {
		return ''
	}
...
fn keg_spec_link_directory(relative string, file string) bool {
	...
	ruby.make_dir_all(directory) or { return false }
	if file != '' {
		ruby.write_file(keg_spec_join(directory, file), '') or { return false }
	}
	fixture.keg.link(false, false) or { return false }
	return ruby.is_dir(keg_spec_join_relative(fixture.prefix, relative))
}
...
pub fn ruby_keg_spec_l8_d1_dst(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return ruby.string_value('') }
	defer { keg_spec_cleanup(fixture) }
	return ruby.object_value('Pathname', fixture.dst)
}

// Ruby let `let(:nonexistent) { Pathname.new("/some/nonexistent/path") }` at line 9.
pub fn ruby_keg_spec_l9_d2_nonexistent(args ...ruby.Value) ruby.Value {
	return ruby.object_value('Pathname', '/some/nonexistent/path')
}

// Ruby let! `let!(:keg) { setup_test_keg("foo", "1.0") }` at line 10.
pub fn ruby_keg_spec_l10_d3_keg(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return ruby.object_value('NilClass', 'nil') }
	defer { keg_spec_cleanup(fixture) }
	return keg_spec_value(fixture.keg)
}

// Ruby let `let(:kegs) { [] }` at line 11.
pub fn ruby_keg_spec_l11_d4_kegs(args ...ruby.Value) ruby.Value {
	return ruby.array_value([]ruby.Value{})
}

// Ruby method `setup_test_keg(name, version)` at line 15.
pub fn ruby_keg_spec_l15_d5_setup_test_keg(args ...ruby.Value) ruby.Value {
	name := if args.len > 0 { args[0].as_string() } else { 'foo' }
	version := if args.len > 1 { args[1].as_string() } else { '1.0' }
	fixture := keg_spec_fixture(name, version) or { return ruby.object_value('NilClass', 'nil') }
	defer { keg_spec_cleanup(fixture) }
	return keg_spec_value(fixture.keg)
}

// Ruby specify `specify "::all" do` at line 38.
pub fn ruby_keg_spec_l38_d6_all(args ...ruby.Value) ruby.Value {
...
pub fn ruby_keg_spec_l42_d7_empty_installation(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return keg_spec_bool(false) }
	defer { keg_spec_cleanup(fixture) }
	for file in ['.DS_Store', homebrew.tab_filename, 'LICENSE.txt'] {
		ruby.write_file(keg_spec_join(fixture.path, file), '') or { return keg_spec_bool(false) }
	}
	initial := !fixture.keg.empty_installation()
	keg_spec_remove(keg_spec_join(fixture.path, 'bin'))
	empty := fixture.keg.empty_installation()
	ruby.make_dir_all(keg_spec_join(fixture.path, 'bin')) or { return keg_spec_bool(false) }
	ruby.write_file(keg_spec_join(fixture.path, 'bin', 'todo'), '') or { return keg_spec_bool(false) }
	return keg_spec_bool(initial && empty && !fixture.keg.empty_installation())
}

// Ruby specify `specify "#oldname_opt_records" do` at line 59.
pub fn ruby_keg_spec_l59_d8_oldname_opt_records(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return keg_spec_bool(false) }
	defer { keg_spec_cleanup(fixture) }
...
pub fn ruby_keg_spec_l66_d9_remove_oldname_opt_records(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return keg_spec_bool(false) }
	defer { keg_spec_cleanup(fixture) }
	record := keg_spec_join(fixture.prefix, 'opt', 'oldfoo')
	...
	preserved := ruby.is_link(record)
	keg_spec_remove(record)
	keg_spec_symlink(fixture.path, record) or { return keg_spec_bool(false) }
	fixture.keg.remove_oldname_opt_records() or { return keg_spec_bool(false) }
	return keg_spec_bool(preserved && !ruby.is_link(record))
}

// Ruby it `it "links a Keg" do` at line 78.
pub fn ruby_keg_spec_l78_d10_links(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return keg_spec_bool(false) }
	defer { keg_spec_cleanup(fixture) }
	count := fixture.keg.link(false, false) or { return keg_spec_bool(false) }
	links := ruby.list_dir(keg_spec_join(fixture.prefix, 'bin')) or { return keg_spec_bool(false) }
	return keg_spec_bool(count == 3 && links.all(ruby.is_link(keg_spec_join(fixture.prefix, 'bin', it)) && !keg_spec_readlink(keg_spec_join(fixture.prefix, 'bin', it)).starts_with('/')))
}

// Ruby let `let(:options) { { dry_run: true } }` at line 86.
pub fn ruby_keg_spec_l86_d11_options(args ...ruby.Value) ruby.Value {
	return ruby.structured_value('Hash', '{dry_run: true}', {
		'dry_run': 'true'
	})
}

// Ruby it `it "only prints what would be done" do` at line 88.
pub fn ruby_keg_spec_l88_d12_only(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return keg_spec_bool(false) }
	defer { keg_spec_cleanup(fixture) }
	count := fixture.keg.link(true, false) or { return keg_spec_bool(false) }
	return keg_spec_bool(count == 0 && !fixture.keg.linked() && !ruby.is_link(fixture.dst))
}

// Ruby it `it "fails when already linked" do` at line 101.
pub fn ruby_keg_spec_l101_d13_fails(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return keg_spec_bool(false) }
	defer { keg_spec_cleanup(fixture) }
...
pub fn ruby_keg_spec_l107_d14_fails(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return keg_spec_bool(false) }
	defer { keg_spec_cleanup(fixture) }
	ruby.write_file(fixture.dst, '') or { return keg_spec_bool(false) }
...
pub fn ruby_keg_spec_l113_d15_ignores(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return keg_spec_bool(false) }
	defer { keg_spec_cleanup(fixture) }
	keg_spec_symlink('/some/nonexistent/path', fixture.dst) or { return keg_spec_bool(false) }
	fixture.keg.link(false, false) or { return keg_spec_bool(false) }
	return keg_spec_bool(ruby.is_link(fixture.dst) && keg_spec_readlink(fixture.dst).contains('Cellar/foo/1.0/bin/helloworld'))
}

// Ruby let `let(:options) { { overwrite: true } }` at line 121.
pub fn ruby_keg_spec_l121_d16_options(args ...ruby.Value) ruby.Value {
	return ruby.structured_value('Hash', '{overwrite: true}', {
		'overwrite': 'true'
	})
}

// Ruby it `it "overwrite existing files" do` at line 123.
pub fn ruby_keg_spec_l123_d17_overwrite(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return keg_spec_bool(false) }
	defer { keg_spec_cleanup(fixture) }
	ruby.write_file(fixture.dst, '') or { return keg_spec_bool(false) }
...
pub fn ruby_keg_spec_l129_d18_overwrites(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return keg_spec_bool(false) }
	defer { keg_spec_cleanup(fixture) }
...
pub fn ruby_keg_spec_l135_d19_still(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return keg_spec_bool(false) }
	defer { keg_spec_cleanup(fixture) }
	ruby.write_file(fixture.dst, '') or { return keg_spec_bool(false) }
	count := fixture.keg.link(true, true) or { return keg_spec_bool(false) }
	return keg_spec_bool(count == 0 && !fixture.keg.linked() && ruby.is_file(fixture.dst))
}

// Ruby it `it "also creates an opt link" do` at line 150.
pub fn ruby_keg_spec_l150_d20_also(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return keg_spec_bool(false) }
	defer { keg_spec_cleanup(fixture) }
...
pub fn ruby_keg_spec_l184_d25_share_pwsh(args ...ruby.Value) ruby.Value {
...
pub fn ruby_keg_spec_l192_d26_symlinks(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return keg_spec_bool(false) }
	defer { keg_spec_cleanup(fixture) }
	example := keg_spec_join(fixture.path, 'lib', 'example')
	link := keg_spec_join(fixture.path, 'lib', 'pkgconfig')
	ruby.make_dir_all(example) or { return keg_spec_bool(false) }
	keg_spec_symlink('example', link) or { return keg_spec_bool(false) }
	fixture.keg.link(false, false) or { return keg_spec_bool(false) }
	target := keg_spec_join(fixture.prefix, 'lib', 'pkgconfig')
	return keg_spec_bool(ruby.is_link(target))
}

// Ruby it `it "unlinks a Keg" do` at line 205.
pub fn ruby_keg_spec_l205_d27_unlinks(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return keg_spec_bool(false) }
	defer { keg_spec_cleanup(fixture) }
	fixture.keg.link(false, false) or { return keg_spec_bool(false) }
	before := ruby.is_link(fixture.dst)
	count := fixture.keg.unlink(false) or { return keg_spec_bool(false) }
	return keg_spec_bool(before && count == 3 && !ruby.is_link(fixture.dst))
}

// Ruby it `it "prunes empty top-level directories" do` at line 212.
pub fn ruby_keg_spec_l212_d28_prunes(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return keg_spec_bool(false) }
	defer { keg_spec_cleanup(fixture) }
	prefix_directory := keg_spec_join(fixture.prefix, 'lib', 'foo', 'bar')
	source_directory := keg_spec_join(fixture.path, 'lib', 'foo', 'bar')
	ruby.make_dir_all(prefix_directory) or { return keg_spec_bool(false) }
	ruby.make_dir_all(source_directory) or { return keg_spec_bool(false) }
	ruby.write_file(keg_spec_join(source_directory, 'file1'), '') or { return keg_spec_bool(false) }
	fixture.keg.link(false, false) or { return keg_spec_bool(false) }
	fixture.keg.unlink(false) or { return keg_spec_bool(false) }
	return keg_spec_bool(!ruby.is_dir(keg_spec_join(fixture.prefix, 'lib', 'foo')))
}

// Ruby it `it "ignores .DS_Store when pruning empty directories" do` at line 222.
pub fn ruby_keg_spec_l222_d29_ignores(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return keg_spec_bool(false) }
	defer { keg_spec_cleanup(fixture) }
	prefix_foo := keg_spec_join(fixture.prefix, 'lib', 'foo')
	source_directory := keg_spec_join(fixture.path, 'lib', 'foo', 'bar')
	ruby.make_dir_all(keg_spec_join(prefix_foo, 'bar')) or { return keg_spec_bool(false) }
	ruby.write_file(keg_spec_join(prefix_foo, '.DS_Store'), '') or { return keg_spec_bool(false) }
	ruby.make_dir_all(source_directory) or { return keg_spec_bool(false) }
	ruby.write_file(keg_spec_join(source_directory, 'file1'), '') or { return keg_spec_bool(false) }
	fixture.keg.link(false, false) or { return keg_spec_bool(false) }
	fixture.keg.unlink(false) or { return keg_spec_bool(false) }
	return keg_spec_bool(!ruby.is_dir(prefix_foo) && !ruby.path_exists(keg_spec_join(prefix_foo, '.DS_Store')))
}

// Ruby it `it "doesn't remove opt link" do` at line 234.
pub fn ruby_keg_spec_l234_d30_doesn(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return keg_spec_bool(false) }
	defer { keg_spec_cleanup(fixture) }
	fixture.keg.link(false, false) or { return keg_spec_bool(false) }
	fixture.keg.unlink(false) or { return keg_spec_bool(false) }
...
pub fn ruby_keg_spec_l240_d31_preverves(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return keg_spec_bool(false) }
	defer { keg_spec_cleanup(fixture) }
	fixture.keg.link(false, false) or { return keg_spec_bool(false) }
	...
	keg_spec_symlink('/some/nonexistent/path', fixture.dst) or { return keg_spec_bool(false) }
	fixture.keg.unlink(false) or { return keg_spec_bool(false) }
	return keg_spec_bool(ruby.is_link(fixture.dst))
}

// Ruby it `it "preverves broken symlinks pointing into the Keg" do` at line 248.
pub fn ruby_keg_spec_l248_d32_preverves(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return keg_spec_bool(false) }
	defer { keg_spec_cleanup(fixture) }
	fixture.keg.link(false, false) or { return keg_spec_bool(false) }
...
pub fn ruby_keg_spec_l255_d33_preverves(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return keg_spec_bool(false) }
	defer { keg_spec_cleanup(fixture) }
	fixture.keg.link(false, false) or { return keg_spec_bool(false) }
	keg_spec_remove(fixture.dst)
...
pub fn ruby_keg_spec_l263_d34_preserves(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return keg_spec_bool(false) }
	defer { keg_spec_cleanup(fixture) }
	fixture.keg.link(false, false) or { return keg_spec_bool(false) }
	keg_spec_remove(fixture.dst)
	ruby.write_file(fixture.dst, '') or { return keg_spec_bool(false) }
	fixture.keg.unlink(false) or { return keg_spec_bool(false) }
	return keg_spec_bool(ruby.is_file(fixture.dst))
}

// Ruby it `it "ignores nonexistent file" do` at line 271.
pub fn ruby_keg_spec_l271_d35_ignores(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return keg_spec_bool(false) }
	defer { keg_spec_cleanup(fixture) }
	fixture.keg.link(false, false) or { return keg_spec_bool(false) }
	keg_spec_remove(fixture.dst)
...
pub fn ruby_keg_spec_l277_d36_doesn(args ...ruby.Value) ruby.Value {
	root := keg_spec_join(ruby.temporary_directory(), 'brew-v-keg-spec-conflict-${ruby.process_id()}')
	ruby.remove_all(root) or {}
	...
	ruby.make_dir_all(keg_spec_join(a_path, 'lib', 'example')) or { return keg_spec_bool(false) }
	keg_spec_symlink('example', keg_spec_join(a_path, 'lib', 'example2')) or { return keg_spec_bool(false) }
	ruby.make_dir_all(keg_spec_join(b_path, 'lib', 'example2')) or { return keg_spec_bool(false) }
	...
	before := (ruby.list_dir(keg_spec_join(prefix, 'lib')) or { return keg_spec_bool(false) }).len
	b.link(false, false) or {
		after := (ruby.list_dir(keg_spec_join(prefix, 'lib')) or { return keg_spec_bool(false) }).len
...
pub fn ruby_keg_spec_l295_d37_removes(args ...ruby.Value) ruby.Value {
	...
	ruby.make_dir_all(source) or { return keg_spec_bool(false) }
	keg_spec_symlink('/some/nonexistent/path', link) or { return keg_spec_bool(false) }
	fixture.keg.link(false, false) or { return keg_spec_bool(false) }
	return keg_spec_bool(ruby.is_dir(link) && !ruby.is_link(link))
}

// Ruby it `it "creates an opt link" do` at line 312.
pub fn ruby_keg_spec_l312_d38_creates(args ...ruby.Value) ruby.Value {
	...
	updated := ruby.real_path(old_record) == ruby.real_path(fixture.path)
	fixture.keg.uninstall() or { return keg_spec_bool(false) }
	return keg_spec_bool(updated && !ruby.is_link(old_record))
}

// Ruby it `it "doesn't fail if already opt-linked" do` at line 324.
pub fn ruby_keg_spec_l324_d39_doesn(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return keg_spec_bool(false) }
	defer { keg_spec_cleanup(fixture) }
	fixture.keg.optlink(false, false) or { return keg_spec_bool(false) }
	fixture.keg.optlink(false, false) or { return keg_spec_bool(false) }
...
pub fn ruby_keg_spec_l330_d40_replaces(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return keg_spec_bool(false) }
	defer { keg_spec_cleanup(fixture) }
	ruby.make_dir_all(fixture.keg.opt_record) or { return keg_spec_bool(false) }
	fixture.keg.optlink(false, false) or { return keg_spec_bool(false) }
	return keg_spec_bool(fixture.keg.optlinked())
}

// Ruby it `it "replaces an existing file" do` at line 336.
pub fn ruby_keg_spec_l336_d41_replaces(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return keg_spec_bool(false) }
	defer { keg_spec_cleanup(fixture) }
	ruby.make_dir_all(keg_spec_parent(fixture.keg.opt_record)) or { return keg_spec_bool(false) }
	ruby.write_file(fixture.keg.opt_record, 'foo') or { return keg_spec_bool(false) }
	fixture.keg.optlink(false, false) or { return keg_spec_bool(false) }
	return keg_spec_bool(fixture.keg.optlinked())
}

// Ruby it `it "identifies Homebrew service files" do` at line 345.
pub fn ruby_keg_spec_l345_d42_identifies(args ...ruby.Value) ruby.Value {
...
pub fn ruby_keg_spec_l367_d43_link(args ...ruby.Value) ruby.Value {
	fixture := keg_spec_fixture('foo', '1.0') or { return keg_spec_bool(false) }
	defer { keg_spec_cleanup(fixture) }
