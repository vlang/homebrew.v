...
pub fn ruby_systemctl_spec_l11_d1_bindir(args ...ruby.Value) ruby.Value {
	return ruby.object_value('Pathname', os.join_path(os.temp_dir(), 'brew-v-systemctl-spec'))
}

// Ruby it `it "outputs systemctl scope for user" do` at line 14.
pub fn ruby_systemctl_spec_l14_d2_outputs(args ...ruby.Value) ruby.Value {
	state := system.new_systemctl_state('', false)
	return ruby.bool_value(state.scope() == '--user')
}

// Ruby it `it "outputs systemctl scope for root" do` at line 19.
pub fn ruby_systemctl_spec_l19_d3_outputs(args ...ruby.Value) ruby.Value {
	state := system.new_systemctl_state('', true)
	return ruby.bool_value(state.scope() == '--system')
}

// Ruby it `it "outputs systemctl command location" do` at line 26.
pub fn ruby_systemctl_spec_l26_d4_outputs(args ...ruby.Value) ruby.Value {
	bindir := os.join_path(os.temp_dir(), 'brew-v-systemctl-spec-${os.getpid()}-${time.now().unix_micro()}')
	os.mkdir_all(bindir) or { return ruby.bool_value(false) }
	...
	os.chmod(executable, 0o755) or { return ruby.bool_value(false) }
	mut state := system.new_systemctl_state(bindir, false)
	return ruby.bool_value((state.executable() or { '' }) == executable)
}
