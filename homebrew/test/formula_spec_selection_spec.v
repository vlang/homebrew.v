...
fn formula_spec_selection(stable_url string, head_url string, requested string) string {
	stable_complete := stable_url != ''
	head_complete := head_url != ''
	if requested == 'head' && head_complete {
		return 'head'
	}
	if requested == 'stable' && stable_complete {
		return 'stable'
	}
	if stable_complete {
		return 'stable'
