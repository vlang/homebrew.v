...
pub fn ruby_dependencies_spec_l8_d1_dependencies(args ...ruby.Value) ruby.Value {
	return homebrew.ruby_dependencies_l15_d1_initialize()
}

// Ruby it `it "returns itself" do` at line 11.
pub fn ruby_dependencies_spec_l11_d2_returns(args ...ruby.Value) ruby.Value {
	mut dependencies := homebrew.new_dependencies()
	result := dependencies.add(homebrew.new_dependency('foo', []string{}))
	return ruby.bool_value(result.equal(dependencies))
}

// Ruby it `it "preserves order" do` at line 15.
pub fn ruby_dependencies_spec_l15_d3_preserves(args ...ruby.Value) ruby.Value {
	names := ['foo', 'bar', 'baz']
	mut dependencies := homebrew.new_dependencies()
	for name in names {
		dependencies.add(homebrew.new_dependency(name, []string{}))
	}
	return ruby.bool_value(dependencies.to_a().map(it.name) == names)
}

// Ruby specify `specify "#*" do` at line 28.
pub fn ruby_dependencies_spec_l28_d4_anonymous(args ...ruby.Value) ruby.Value {
	dependencies := homebrew.new_dependencies(homebrew.new_dependency('foo', []string{}), homebrew.new_dependency('bar', []string{}))
	return ruby.bool_value(dependencies.join(', ') == 'foo, bar')
}

// Ruby specify `specify "#to_a" do` at line 34.
pub fn ruby_dependencies_spec_l34_d5_to_a(args ...ruby.Value) ruby.Value {
	dependency := homebrew.new_dependency('foo', []string{})
	dependencies := homebrew.new_dependencies(dependency)
	values := dependencies.to_a()
	return ruby.bool_value(values.len == 1 && values[0].equal(dependency))
}

// Ruby specify `specify "#to_ary" do` at line 40.
pub fn ruby_dependencies_spec_l40_d6_to_ary(args ...ruby.Value) ruby.Value {
	dependency := homebrew.new_dependency('foo', []string{})
	dependencies := homebrew.new_dependencies(dependency)
...
pub fn ruby_dependencies_spec_l46_d7_type(args ...ruby.Value) ruby.Value {
	...
	return ruby.bool_value(dependencies.required().map(it.name) == ['foo', 'quux']
...
pub fn ruby_dependencies_spec_l60_d8_equality(args ...ruby.Value) ruby.Value {
	dependency := homebrew.new_dependency('foo', []string{})
	...
	return ruby.bool_value(equal_before && !a.equal(b))
}

// Ruby specify `specify "#empty?" do` at line 78.
pub fn ruby_dependencies_spec_l78_d9_empty(args ...ruby.Value) ruby.Value {
	mut dependencies := homebrew.new_dependencies()
	empty_before := dependencies.empty()
	dependencies.add(homebrew.new_dependency('foo', []string{}))
	return ruby.bool_value(empty_before && !dependencies.empty())
}

// Ruby specify `specify "#inspect" do` at line 85.
pub fn ruby_dependencies_spec_l85_d10_inspect(args ...ruby.Value) ruby.Value {
	mut dependencies := homebrew.new_dependencies()
	empty_inspect := dependencies.inspect()
	dependencies.add(homebrew.new_dependency('foo', []string{}))
	return ruby.bool_value(empty_inspect == '#<Dependencies: []>'
