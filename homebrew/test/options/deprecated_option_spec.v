...
pub fn ruby_deprecated_option_spec_l7_d1_option(args ...ruby.Value) ruby.Value {
	...
	return ruby.structured_value('DeprecatedOption', '${old}:${current}', {
		'old':          option.old
...
pub fn ruby_deprecated_option_spec_l9_d2_do(args ...ruby.Value) ruby.Value {
	option := option_types.new_deprecated_option('foo', 'bar')
	return ruby.bool_value(option.old == 'foo' && option.old_flag() == '--foo'
		&& option.current == 'bar' && option.current_flag() == '--bar')
}

// Ruby specify `specify "equality" do` at line 16.
pub fn ruby_deprecated_option_spec_l16_d3_equality(args ...ruby.Value) ruby.Value {
	option := option_types.new_deprecated_option('foo', 'bar')
	foobar := option_types.new_deprecated_option('foo', 'bar')
	boofar := option_types.new_deprecated_option('boo', 'far')
	return ruby.bool_value(foobar.equal(option) && option.equal(foobar)
