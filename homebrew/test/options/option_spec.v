...
pub fn ruby_option_spec_l7_d1_option(args ...ruby.Value) ruby.Value {
	...
	return ruby.structured_value('Option', option.str(), {
		'name':        option.name
		'flag':        option.flag
...
pub fn ruby_option_spec_l9_d2_do(args ...ruby.Value) ruby.Value {
	option := option_types.new_option('foo')
	described := option_types.new_option('foo', 'foo')
	return ruby.bool_value(option.str() == '--foo' && option.description == ''
		&& described.description == 'foo' && option.inspect() == '#<Option: "--foo">')
}

// Ruby specify `specify "equality" do` at line 16.
pub fn ruby_option_spec_l16_d3_equality(args ...ruby.Value) ruby.Value {
	option := option_types.new_option('foo')
	foo := option_types.new_option('foo')
	bar := option_types.new_option('bar')
	return ruby.bool_value(option.equal(foo) && !option.equal(bar))
}
