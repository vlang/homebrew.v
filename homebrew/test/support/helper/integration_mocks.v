...
pub fn ruby_integration_mocks_l7_d1_check_integration_test(args ...brew_runtime.Value) brew_runtime.Value {
	enabled := if args.len > 0 {
		...
		brew_runtime.environment_value('HOMEBREW_INTEGRATION_TEST') != ''
	}
	if finding := check_integration_test(enabled) {
		return brew_runtime.structured_value('Homebrew::Diagnostic::Finding', finding.message, {
...
pub fn ruby_integration_mocks_l13_d2_exec(args ...brew_runtime.Value) brew_runtime.Value {
	...
	coverage := if args.len > 1 {
		...
		brew_runtime.environment_value('HOMEBREW_TESTS_COVERAGE') != ''
	}
	...
	return brew_runtime.map_value({
		...
		'flush_coverage': brew_runtime.bool_value(plan.flush_coverage)
		'action':         brew_runtime.string_value('Kernel.exec')
	})
}
