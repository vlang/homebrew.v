...
pub fn ruby_cleanup_subcommand_spec_l10_d1_root(args ...ruby.Value) ruby.Value {
	result := service_subcommand.service_cleanup(service_subcommand.ServiceSubcommandRequest{
		root: true
	})
	return ruby.bool_value(result.output == 'All root services OK, nothing cleaned...\n')
}

// Ruby it `it "user - prints on empty cleanup" do` at line 20.
pub fn ruby_cleanup_subcommand_spec_l20_d2_user(args ...ruby.Value) ruby.Value {
	result := service_subcommand.service_cleanup(service_subcommand.ServiceSubcommandRequest{})
	return ruby.bool_value(result.output == 'All user-space services OK, nothing cleaned...\n')
}

// Ruby it `it "prints nothing on cleanup" do` at line 30.
pub fn ruby_cleanup_subcommand_spec_l30_d3_prints(args ...ruby.Value) ruby.Value {
	result := service_subcommand.service_cleanup(service_subcommand.ServiceSubcommandRequest{
		orphaned: ['a']
		unused: ['b']
	})
	return ruby.bool_value(result.output == '' && result.cleaned == ['a', 'b'])
}
