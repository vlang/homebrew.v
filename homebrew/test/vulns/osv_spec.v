...
import homebrew.vulns as osv_core
import x.json2

// Translated from Homebrew/brew `test/vulns/osv_spec.rb`.
// The original source is retained below until every stub has a typed V body.
pub struct OsvSpecStub {
pub:
	results []osv_core.OsvHttpResult
}

fn osv_spec_batch_result(results []osv_core.OsvBatchResult) osv_core.OsvHttpResult {
	return osv_core.OsvHttpResult{
		stdout: json2.encode(osv_core.OsvBatchResponse{
			results: results
		})
	}
}

fn osv_spec_post_payload(arguments []string) !osv_core.OsvBatchRequest {
	json_index := arguments.index('--json')
	if json_index < 0 || json_index + 1 >= arguments.len {
		return error('missing --json payload')
	}
	return json2.decode[osv_core.OsvBatchRequest](arguments[json_index + 1])!
}

fn osv_spec_vulnerability(id string) osv_core.OsvVulnerability {
	return osv_core.OsvVulnerability{
		id: id
	}
}

fn osv_spec_aligned_request(_ []string) !osv_core.OsvHttpResult {
	return osv_spec_batch_result([
		osv_core.OsvBatchResult{
			vulns: [osv_spec_vulnerability('CVE-2024-1111')]
		},
		osv_core.OsvBatchResult{},
		osv_core.OsvBatchResult{
			vulns: [osv_spec_vulnerability('CVE-2024-2222'), osv_spec_vulnerability('CVE-2024-3333')]
		},
	])
}

fn osv_spec_posted_packages_request(arguments []string) !osv_core.OsvHttpResult {
	expected_prefix := ['--fail', '--location', '--silent', '--json']
	if arguments.len < 8 || arguments[..4] != expected_prefix || arguments[arguments.len - 3..] != [
		'--request',
		'POST',
		'${osv_core.osv_api_base}/querybatch',
	] {
		return error('unexpected POST arguments: ${arguments}')
	}
	payload := osv_spec_post_payload(arguments)!
	if payload.queries.len != 3 {
		return error('expected three queries')
	}
	first := payload.queries[0]
	second := payload.queries[1]
	third := payload.queries[2]
	if first.package.name != 'https://github.com/a/a' || first.package.ecosystem != 'GIT' || first.version or {
		''} != 'v1' || second.package.name != 'requests' || second.package.ecosystem != 'PyPI' || second.version or {
		''} != '2.31.0' || third.package.name != 'curl' || third.package.ecosystem != 'Debian' || third.version != none {
		return error('query package payload did not match')
	}
	return osv_spec_batch_result([osv_core.OsvBatchResult{}, osv_core.OsvBatchResult{},
		osv_core.OsvBatchResult{}])
}

fn osv_spec_unexpected_request(_ []string) !osv_core.OsvHttpResult {
	return error('network request was not expected')
}

fn osv_spec_batching_request(arguments []string) !osv_core.OsvHttpResult {
	payload := osv_spec_post_payload(arguments)!
	mut results := []osv_core.OsvBatchResult{cap: payload.queries.len}
	for query in payload.queries {
		id := query.package.name.all_after_last('/').to_upper()
		results << osv_core.OsvBatchResult{
			vulns: [osv_spec_vulnerability(id)]
		}
	}
	return osv_spec_batch_result(results)
}

fn osv_spec_missing_results_request(_ []string) !osv_core.OsvHttpResult {
	return osv_core.OsvHttpResult{
		stdout: '{}'
	}
}

fn osv_spec_short_results_request(_ []string) !osv_core.OsvHttpResult {
	return osv_spec_batch_result([osv_core.OsvBatchResult{}, osv_core.OsvBatchResult{}])
}

fn osv_spec_truncated_continuation_request(arguments []string) !osv_core.OsvHttpResult {
	payload := osv_spec_post_payload(arguments)!
	if payload.queries.len > 0 && payload.queries[0].page_token != none {
		return osv_spec_batch_result([osv_core.OsvBatchResult{
			vulns: [osv_spec_vulnerability('A2')]
		}])
	}
	return osv_spec_batch_result([
		osv_core.OsvBatchResult{
			vulns: [osv_spec_vulnerability('A1')]
			next_page_token: 'tok-a'
		},
		osv_core.OsvBatchResult{
			vulns: [osv_spec_vulnerability('B1')]
			next_page_token: 'tok-b'
		},
		osv_core.OsvBatchResult{
			vulns: [osv_spec_vulnerability('C1')]
		},
	])
}

fn osv_spec_paginated_request(arguments []string) !osv_core.OsvHttpResult {
	payload := osv_spec_post_payload(arguments)!
	if payload.queries.len == 1 && payload.queries[0].page_token != none {
		query := payload.queries[0]
		if query.package.name != 'https://github.com/b/b' || query.package.ecosystem != 'GIT' || query.version or {
			''} != 'v2' || query.page_token or { '' } != 'tok-b' {
			return error('continuation query did not carry its package, version and token')
		}
		return osv_spec_batch_result([osv_core.OsvBatchResult{
			vulns: [osv_spec_vulnerability('B2'), osv_spec_vulnerability('B3')]
		}])
	}
	return osv_spec_batch_result([
		osv_core.OsvBatchResult{
...
fn osv_spec_looping_request(_ []string) !osv_core.OsvHttpResult {
	return osv_spec_batch_result([osv_core.OsvBatchResult{
		vulns: [osv_spec_vulnerability('LOOP')]
		next_page_token: 'again'
	}])
}

fn osv_spec_failed_request(_ []string) !osv_core.OsvHttpResult {
	return osv_core.OsvHttpResult{
		stdout: 'server on fire'
		success: false
		exit_status: 22
	}
}

fn osv_spec_invalid_json_request(_ []string) !osv_core.OsvHttpResult {
	return osv_core.OsvHttpResult{
		stdout: '<html>not json</html>'
	}
}

fn osv_spec_vulnerability_request(arguments []string) !osv_core.OsvHttpResult {
	if arguments != [
		'--fail',
		'--location',
		'--silent',
		'${osv_core.osv_api_base}/vulns/CVE-2024-1234',
	] {
		return error('unexpected GET arguments: ${arguments}')
	}
	return osv_core.OsvHttpResult{
		stdout: json2.encode(osv_core.OsvVulnerability{
			id: 'CVE-2024-1234'
...
fn osv_spec_encoded_vulnerability_request(arguments []string) !osv_core.OsvHttpResult {
	if arguments.len == 0 || arguments.last() != '${osv_core.osv_api_base}/vulns/GO-2024-1%2F2' {
		return error('vulnerability ID was not URL encoded')
	}
	return osv_core.OsvHttpResult{
		stdout: json2.encode(osv_core.OsvVulnerability{
			id: 'GO-2024-1/2'
...
pub fn ruby_osv_spec_l7_d1_curl_result(stdout string, success bool) osv_core.OsvHttpResult {
	return osv_core.OsvHttpResult{
		stdout: stdout
		success: success
		exit_status: if success { 0 } else { 22 }
	}
}

// Ruby method `stub_curl(*results)` at line 11.
pub fn ruby_osv_spec_l11_d2_stub_curl(results []osv_core.OsvHttpResult) OsvSpecStub {
	return OsvSpecStub{
...
pub fn ruby_osv_spec_l16_d3_packages() []osv_core.OsvPackage {
	return [
		osv_core.OsvPackage{'GIT', 'https://github.com/a/a', 'v1'},
		osv_core.OsvPackage{'GIT', 'https://github.com/b/b', 'v2'},
		osv_core.OsvPackage{'GIT', 'https://github.com/c/c', 'v3'},
	]
}

// Ruby it `it "returns an array of vuln arrays aligned with the input" do` at line 24.
pub fn ruby_osv_spec_l24_d4_returns() !bool {
	results := osv_core.osv_query_batch(ruby_osv_spec_l16_d3_packages(), osv_spec_aligned_request)!
	return results.len == 3 && results[0].map(it.id) == ['CVE-2024-1111'] && results[1].len == 0 && results[2].map(it.id) == [
		'CVE-2024-2222',
		'CVE-2024-3333',
	]
}
...
pub fn ruby_osv_spec_l42_d5_posts() !bool {
	mixed := [
		osv_core.OsvPackage{'GIT', 'https://github.com/a/a', 'v1'},
		osv_core.OsvPackage{'PyPI', 'requests', '2.31.0'},
		osv_core.OsvPackage{
			ecosystem: 'Debian'
			name: 'curl'
		},
	]
	results := osv_core.osv_query_batch(mixed, osv_spec_posted_packages_request)!
	return results.len == 3
}

// Ruby it `it "returns empty for empty input without hitting the network" do` at line 64.
pub fn ruby_osv_spec_l64_d6_returns() !bool {
	return osv_core.osv_query_batch([]osv_core.OsvPackage{}, osv_spec_unexpected_request)! == []
}

// Ruby it `it "splits requests larger than BATCH_SIZE and reassembles results in order" do` at line 69.
pub fn ruby_osv_spec_l69_d7_splits() !bool {
	results := osv_core.osv_query_batch_with_limits(ruby_osv_spec_l16_d3_packages(), 2, osv_core.osv_max_pages, osv_spec_batching_request)!
	return results.map(if it.len > 0 { it[0].id } else { '' }) == ['A', 'B', 'C']
}

// Ruby it `it "raises ApiError when the results key is missing" do` at line 81.
pub fn ruby_osv_spec_l81_d8_raises() bool {
	if _ := osv_core.osv_query_batch(ruby_osv_spec_l16_d3_packages(), osv_spec_missing_results_request) {
		return false
	} else {
		return err.msg().contains('expected 3 results')
	}
}

// Ruby it `it "raises ApiError when fewer results than queries are returned" do` at line 87.
pub fn ruby_osv_spec_l87_d9_raises() bool {
	if _ := osv_core.osv_query_batch(ruby_osv_spec_l16_d3_packages(), osv_spec_short_results_request) {
		return false
	} else {
		return err.msg().contains('expected 3 results, got 2')
	}
}

// Ruby it `it "raises ApiError when a continuation response is truncated" do` at line 94.
pub fn ruby_osv_spec_l94_d10_raises() bool {
	if _ := osv_core.osv_query_batch(ruby_osv_spec_l16_d3_packages(), osv_spec_truncated_continuation_request) {
		return false
	} else {
		return err.msg().contains('expected 2 results, got 1')
	}
}

// Ruby it `it "follows per-result next_page_token, resubmitting only paged queries" do` at line 108.
pub fn ruby_osv_spec_l108_d11_follows() !bool {
	results := osv_core.osv_query_batch(ruby_osv_spec_l16_d3_packages(), osv_spec_paginated_request)!
	return results.len == 3 && results[0].map(it.id) == ['A1'] && results[1].map(it.id) == [
		'B1',
		'B2',
		'B3',
	] && results[2].map(it.id) == ['C1']
}

// Ruby it `it "raises after MAX_PAGES continuation requests" do` at line 135.
pub fn ruby_osv_spec_l135_d12_raises() bool {
	if _ := osv_core.osv_query_batch_with_limits([ruby_osv_spec_l16_d3_packages()[0]], 1000, 3, osv_spec_looping_request) {
		return false
	} else {
		return err.msg().contains('more than 3 pages')
	}
}

// Ruby it `it "raises ApiError when curl reports failure" do` at line 144.
pub fn ruby_osv_spec_l144_d13_raises() bool {
	if _ := osv_core.osv_query_batch(ruby_osv_spec_l16_d3_packages(), osv_spec_failed_request) {
		return false
	} else {
		return err.msg().contains('OSV API') && err.msg().contains('curl exit 22')
	}
}

// Ruby it `it "raises ApiError when the response is not valid JSON" do` at line 150.
pub fn ruby_osv_spec_l150_d14_raises() bool {
	if _ := osv_core.osv_query_batch(ruby_osv_spec_l16_d3_packages(), osv_spec_invalid_json_request) {
		return false
	} else {
		return err.msg().contains('Invalid JSON')
	}
}

// Ruby it `it "fetches a single vulnerability record by id" do` at line 158.
pub fn ruby_osv_spec_l158_d15_fetches() !bool {
	vulnerability := osv_core.osv_vulnerability('CVE-2024-1234', osv_spec_vulnerability_request)!
	return vulnerability.id == 'CVE-2024-1234' && vulnerability.summary == 'Test vulnerability' && vulnerability.details == 'Full details here'
}

// Ruby it `it "URL-encodes the id in the request path" do` at line 174.
pub fn ruby_osv_spec_l174_d16_url_encodes() !bool {
	vulnerability := osv_core.osv_vulnerability('GO-2024-1/2', osv_spec_encoded_vulnerability_request)!
	return vulnerability.id == 'GO-2024-1/2'
}

// Ruby it `it "raises ApiError when curl reports failure" do` at line 186.
pub fn ruby_osv_spec_l186_d17_raises() bool {
	if _ := osv_core.osv_vulnerability('CVE-0000-0000', osv_spec_failed_request) {
		return false
	} else {
		return err.msg().contains('OSV API') && err.msg().contains('curl exit 22')
	}
