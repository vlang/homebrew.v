...
pub fn ruby_finding_spec_l9_d1_returns(args ...ruby.Value) ruby.Value {
	_ = args
	return ruby.bool_value(core.new_remediation([], '').string() == '')
}

// Ruby it `it "prefers the text over commands" do` at line 13.
pub fn ruby_finding_spec_l13_d2_prefers(args ...ruby.Value) ruby.Value {
	_ = args
	return ruby.bool_value(core.new_remediation(['brew fix'], 'Do this instead').string() == 'Do this instead')
}

// Ruby it `it "formats commands when no text is given" do` at line 18.
pub fn ruby_finding_spec_l18_d3_formats(args ...ruby.Value) ruby.Value {
	_ = args
	return ruby.bool_value(core.new_remediation(['brew fix', 'brew doctor'], '').string() == 'You can solve this by running:\n  brew fix\n  brew doctor')
}

// Ruby it `it "returns the commands and text" do` at line 25.
pub fn ruby_finding_spec_l25_d4_returns(args ...ruby.Value) ruby.Value {
	_ = args
	value := core.new_remediation(['brew fix'], 'Do this').to_value().as_map() or {
		return ruby.bool_value(false)
	}
	commands := value['commands'].as_string_array() or { return ruby.bool_value(false) }
	return ruby.bool_value(commands == ['brew fix'] && value['text'].as_string() == 'Do this')
}

// Ruby it `it "wraps a string remediation in a Remediation" do` at line 33.
pub fn ruby_finding_spec_l33_d5_wraps(args ...ruby.Value) ruby.Value {
	_ = args
	finding := core.new_finding('Something is wrong', '1', [], [], core.Remediation{
		text: 'Fix it'
	})
	remediation := finding.remediation or { return ruby.bool_value(false) }
	return ruby.bool_value(remediation.text == 'Fix it')
}

// Ruby it `it "keeps a Remediation remediation as-is" do` at line 38.
pub fn ruby_finding_spec_l38_d6_keeps(args ...ruby.Value) ruby.Value {
	_ = args
	remediation := core.Remediation{
		text: 'Fix it'
	}
	finding := core.new_finding('Something is wrong', '1', [], [], remediation)
	kept := finding.remediation or { return ruby.bool_value(false) }
	return ruby.bool_value(kept == remediation)
}

// Ruby it `it "defaults remediation to nil and tier to 1" do` at line 44.
pub fn ruby_finding_spec_l44_d7_defaults(args ...ruby.Value) ruby.Value {
	_ = args
	finding := core.new_finding('Something is wrong', '', [], [], none)
	return ruby.bool_value(finding.remediation == none && finding.tier == '1')
}

// Ruby it `it "serialises all attributes" do` at line 52.
pub fn ruby_finding_spec_l52_d8_serialises(args ...ruby.Value) ruby.Value {
	_ = args
	finding := core.new_finding('Something is wrong', '2', ['foo'], ['https://brew.sh'], core.Remediation{
		text: 'Fix it'
	})
	value := finding.to_value().as_map() or { return ruby.bool_value(false) }
	remediation := value['remediation'].as_map() or { return ruby.bool_value(false) }
	affects := value['affects'].as_string_array() or { return ruby.bool_value(false) }
	links := value['links'].as_string_array() or { return ruby.bool_value(false) }
	commands := remediation['commands'].as_string_array() or {
		return ruby.bool_value(false)
	}
	return ruby.bool_value(value['text'].as_string() == 'Something is wrong' && value['tier'].as_string() == '2' && affects == [
		'foo',
...
pub fn ruby_finding_spec_l69_d9_serialises(args ...ruby.Value) ruby.Value {
	_ = args
	value := core.new_finding('Something is wrong', '', [], [], none).to_value().as_map() or {
		return ruby.bool_value(false)
	}
	return ruby.bool_value(value['remediation'].type_name == 'NilClass')
}

// Ruby it `it "includes only the text for a Tier 1 finding without remediation" do` at line 75.
pub fn ruby_finding_spec_l75_d10_includes(args ...ruby.Value) ruby.Value {
	_ = args
	return ruby.bool_value(core.new_finding('Something is wrong', '1', [], [], none).string() == 'Something is wrong')
}

// Ruby it `it "includes the remediation text" do` at line 79.
pub fn ruby_finding_spec_l79_d11_includes(args ...ruby.Value) ruby.Value {
	_ = args
	finding := core.new_finding('Something is wrong', '1', [], [], core.Remediation{
		text: 'Fix it'
	})
	return ruby.bool_value(finding.string() == 'Something is wrong\nFix it')
}

// Ruby it `it "returns nil for Tier 1" do` at line 90.
pub fn ruby_finding_spec_l90_d12_returns(args ...ruby.Value) ruby.Value {
	_ = args
	return ruby.bool_value(core.support_tier_message('1', false, '') == '')
}

// Ruby it `it "links to the tier-specific documentation" do` at line 94.
pub fn ruby_finding_spec_l94_d13_links(args ...ruby.Value) ruby.Value {
	_ = args
	return ruby.bool_value(core.support_tier_message('2', false, '').contains('https://docs.brew.sh/Support-Tiers#tier-2'))
}

// Ruby it `it "describes unsupported configurations" do` at line 99.
pub fn ruby_finding_spec_l99_d14_describes(args ...ruby.Value) ruby.Value {
	_ = args
	message := core.support_tier_message('unsupported', false, '')
	return ruby.bool_value(message.contains('This is a Unsupported configuration:') && message.contains('https://docs.brew.sh/Support-Tiers#unsupported'))
}

// Ruby it `it "points Nix-managed installs at the upstream Nix project" do` at line 105.
pub fn ruby_finding_spec_l105_d15_points(args ...ruby.Value) ruby.Value {
	_ = args
	return ruby.bool_value(core.support_tier_message('2', true, '').contains('Report issues to the upstream Nix project, not'))
}
