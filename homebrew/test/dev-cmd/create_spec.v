...
pub fn ruby_create_spec_l8_d1_url(args ...brew_runtime.Value) brew_runtime.Value {
	...
	return brew_runtime.string_value('file://${fixture_dir}/tarballs/testball-0.1.tbz')
}
...
pub fn ruby_create_spec_l9_d2_formula_file(args ...brew_runtime.Value) brew_runtime.Value {
	...
	return brew_runtime.object_value('Pathname', os.join_path(tap_path, 'Formula', 'testball.rb'))
}
...
pub fn ruby_create_spec_l13_d3_creates(args ...brew_runtime.Value) brew_runtime.Value {
	if args.len == 0 {
		return brew_runtime.object_value('ArgumentError', 'temporary tap path is required')
	}
	root := args[0].as_string()
	path := os.join_path(root, 'Formula', 'testball.rb')
	result := run_create(CreateOptions{
		url: ruby_create_spec_l8_d1_url(brew_runtime.string_value(root)).as_string()
		set_name: 'Testball'
		tap_path: root
		formula_path: path
	...
	return brew_runtime.bool_value(result.editor_path == path && os.exists(path)
...
pub fn ruby_create_spec_l20_d4_generates(args ...brew_runtime.Value) brew_runtime.Value {
	return brew_runtime.bool_value(create_cask_token('A FooBar_Baz++!') == 'a-foobar-baz-plus-plus')
}
...
pub fn ruby_create_spec_l25_d5_retains(args ...brew_runtime.Value) brew_runtime.Value {
	return brew_runtime.bool_value(create_cask_token('test@preview') == 'test@preview')
}
