...
pub fn ruby_desc_helper_l21_d1_audit_desc(args ...ruby.Value) ruby.Value {
	...
	has_desc := if args.len > 3 { args[3].as_bool() or { true } } else { true }
	begin_pos := if args.len > 4 { int(args[4].int_data) } else { 0 }
	end_pos := if args.len > 5 { int(args[5].int_data) } else { description.len + 2 }
	call := DescCall{
		description: description
		begin_pos: begin_pos
		end_pos: end_pos
		literal_begin_pos: begin_pos
		literal_end_pos: end_pos
