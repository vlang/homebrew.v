...
pub fn ruby_url_l17_d1_on_url_stanza(args ...ruby.Value) ruby.Value {
	...
	mut offenses := []CaskUrlOffense{}
	if source.contains('cask ') || source.contains('cask(') {
		offenses = audit_cask_url(source, path)
	} else {
		stanza := parse_cask_url_stanza(source, 0) or { CaskUrlStanza{} }
		if stanza.source != '' {
			offenses = audit_parsed_cask_url(stanza, cask_url_tap(path), false)
		}
	}
