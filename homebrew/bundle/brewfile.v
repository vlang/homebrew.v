...
pub fn real_brewfile_reader(path string) !string {
	return brew_runtime.read_file(path)!
}
...
pub fn current_bundle_brewfile_path_config(global bool, file ?string) BundleBrewfilePathConfig {
	...
	home_directory := brew_runtime.environment_value_opt('HOME') or { working_directory }
	user_config_home := brew_runtime.environment_value('HOMEBREW_USER_CONFIG_HOME')
	home_brewfile := brew_runtime.join_path(home_directory, '.Brewfile')
	...
	return BundleBrewfilePathConfig{
		...
		user_config_home_exists: user_config_home != '' && brew_runtime.is_dir(user_config_home)
		user_config_brewfile_exists: user_config_home != '' && brew_runtime.path_exists(user_config_brewfile)
		home_brewfile_exists: brew_runtime.path_exists(home_brewfile)
	}
}

fn brewfile_path_config_from_value(value brew_runtime.Value) BundleBrewfilePathConfig {
...
fn brewfile_boundary_config(args []brew_runtime.Value, config_index int) BundleBrewfilePathConfig {
...
pub fn ruby_brewfile_l63_d3_self_handle_file_value(args ...brew_runtime.Value) brew_runtime.Value {
	if args.len == 0 {
		return brew_runtime.object_value('ArgumentError', 'filename is required')
	}
	dash_writes_to_stdout := if args.len > 1 { args[1].as_bool() or { false } } else { false }
	return brew_runtime.string_value(handle_brewfile_file_value(args[0].as_string(), dash_writes_to_stdout))
}
