...
pub fn ruby_help_l22_d1_self_help(request HelpRequest, context HelpContext) !HelpResult {
	return help(request, context)
}

// Ruby method `self.command_help(cmd, path, remaining_args:, usage_error:)` at line 71.
pub fn ruby_help_l71_d2_self_command_help(command HelpCommandMetadata,
	remaining_args []string, usage_error bool, context HelpContext) !string {
	return (command_help(command, remaining_args, usage_error, context)!).output
}

// Ruby method `self.parser_help(path, remaining_args:, usage_error:)` at line 101.
pub fn ruby_help_l101_d3_self_parser_help(command HelpCommandMetadata,
	remaining_args []string, usage_error bool, render HelpParserRenderer) !HelpText {
	return parser_help(command, remaining_args, usage_error, render)
}

// Ruby method `self.command_help_lines(path)` at line 114.
pub fn ruby_help_l114_d4_self_command_help_lines(path string) ![]string {
	return command_help_lines(path)
}

// Ruby method `self.comment_help(path)` at line 123.
pub fn ruby_help_l123_d5_self_comment_help(path string, style HelpStyle) !HelpText {
	return comment_help(path, style)
}
