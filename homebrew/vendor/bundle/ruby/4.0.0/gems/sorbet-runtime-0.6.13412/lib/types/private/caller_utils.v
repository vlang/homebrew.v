...
fn caller_location_from_value(value brew_runtime.Value) CallerLocation {
...
fn caller_location_value(location CallerLocation) brew_runtime.Value {
	return brew_runtime.structured_value('Thread::Backtrace::Location', location.path, {
		'path':   location.path
...
fn caller_boundary(args []brew_runtime.Value, skip_first bool) brew_runtime.Value {
	if args.len == 0 {
		return brew_runtime.Value{ type_name: 'NilClass', repr: 'nil' }
	}
	values := args[0].as_array() or { return brew_runtime.Value{ type_name: 'NilClass', repr: 'nil' } }
	...
	}) or { return brew_runtime.Value{ type_name: 'NilClass', repr: 'nil' } }
