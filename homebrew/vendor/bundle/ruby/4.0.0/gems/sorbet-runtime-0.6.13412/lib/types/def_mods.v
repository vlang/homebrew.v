...
pub:
	owner              ruby.Value
	method_name        ruby.Value
	kind               DefModifierKind
	allow_incompatible ruby.Value
}

pub struct DefModifierResult {
pub:
	method_name ruby.Value
	declaration ruby.Value
}
...
pub fn apply_def_modifier(request DefModifierRequest) !DefModifierResult {
	...
	if previous.map_data['block'] or { ruby.object_value('NilClass', 'nil') }.type_name != 'Proc' {
...
fn def_modifier_boundary(args []ruby.Value, kind DefModifierKind) ruby.Value {
	if args.len < 2 {
