...
pub type DeclStateAction = fn(mut DeclState) !ruby.Value
...
fn new_decl_state() &DeclState {
	return &DeclState{
		active_declaration: ruby.object_value('NilClass', 'nil')
		previous_declaration: ruby.object_value('NilClass', 'nil')
	}
}
...
pub fn (mut state DeclState) reset() {
	state.active_declaration = ruby.object_value('NilClass', 'nil')
	state.previous_declaration = ruby.object_value('NilClass', 'nil')
}

pub fn (mut state DeclState) consume() ruby.Value {
	state.previous_declaration = state.active_declaration
	state.active_declaration = ruby.object_value('NilClass', 'nil')
	return state.previous_declaration
}

pub fn (mut state DeclState) without_on_method_added(action DeclStateAction) !ruby.Value {
...
fn decl_state_value(state &DeclState) ruby.Value {
	return ruby.structured_value('T::Private::DeclState', '#<T::Private::DeclState>', {
		'decl_state_address': u64(voidptr(state)).str()
	})
}

fn decl_state_from_value(value ruby.Value) &DeclState {
...
fn decl_state_from_args(args []ruby.Value) &DeclState {
	if args.len == 0 {
...
pub fn ruby_decl_state_l5_d1_self_current(args ...ruby.Value) ruby.Value {
	return decl_state_value(current_decl_state())
}

// Ruby method `self.current=(other)` at line 9.
pub fn ruby_decl_state_l9_d2_self_current(args ...ruby.Value) ruby.Value {
	if args.len == 0 {
...
pub fn ruby_decl_state_l13_d3_active_declaration(args ...ruby.Value) ruby.Value {
	return decl_state_from_args(args).active_declaration
}

// Ruby attr_accessor `attr_accessor :active_declaration` at line 13.
pub fn ruby_decl_state_l13_d4_active_declaration(args ...ruby.Value) ruby.Value {
	if args.len < 2 {
...
pub fn ruby_decl_state_l14_d5_skip_on_method_added(args ...ruby.Value) ruby.Value {
	return ruby.bool_value(decl_state_from_args(args).skip_on_method_added)
}

// Ruby attr_accessor `attr_accessor :skip_on_method_added` at line 14.
pub fn ruby_decl_state_l14_d6_skip_on_method_added(args ...ruby.Value) ruby.Value {
	if args.len < 2 {
...
pub fn ruby_decl_state_l15_d7_previous_declaration(args ...ruby.Value) ruby.Value {
	return decl_state_from_args(args).previous_declaration
}

// Ruby attr_accessor `attr_accessor :previous_declaration` at line 15.
pub fn ruby_decl_state_l15_d8_previous_declaration(args ...ruby.Value) ruby.Value {
	if args.len < 2 {
...
pub fn ruby_decl_state_l17_d9_reset(args ...ruby.Value) ruby.Value {
	mut state := decl_state_from_args(args)
	state.reset()
...
pub fn ruby_decl_state_l22_d10_consume(args ...ruby.Value) ruby.Value {
	mut state := decl_state_from_args(args)
	return state.consume()
}

// Ruby method `without_on_method_added` at line 28.
pub fn ruby_decl_state_l28_d11_without_on_method_added(args ...ruby.Value) ruby.Value {
	mut state := decl_state_from_args(args)
	old_value := state.skip_on_method_added
	state.skip_on_method_added = true
