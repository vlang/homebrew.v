...
pub fn new_power_of_two_tuple(size int) !&PowerOfTwoTuple {
	...
	return &PowerOfTwoTuple{
		size: size
		values: []brew_runtime.Value{len: size}
		set: []bool{len: size}
	}
}
...
pub fn (mut tuple PowerOfTwoTuple) get_by_hash(hash i64) ?brew_runtime.Value {
...
pub fn (mut tuple PowerOfTwoTuple) set_by_hash(hash i64, value brew_runtime.Value) brew_runtime.Value {
	index := tuple.hash_to_index(hash)
	tuple.lock.lock()
...
fn power_tuple_boundary_new(size int) brew_runtime.Value {
	tuple := new_power_of_two_tuple(size) or { panic(err) }
	return brew_runtime.structured_value('Concurrent::ThreadSafe::Util::PowerOfTwoTuple', '#<Concurrent::ThreadSafe::Util::PowerOfTwoTuple>', {
		'power_tuple_address': u64(voidptr(tuple)).str()
		'size':                size.str()
	})
}

fn power_tuple_boundary_receiver(args []brew_runtime.Value) &PowerOfTwoTuple {
	if args.len == 0 {
...
pub fn ruby_power_of_two_tuple_l20_d2_hash_to_index(args ...brew_runtime.Value) brew_runtime.Value {
	...
	return brew_runtime.int_value(tuple.hash_to_index(args[1].as_int() or { panic(err) }))
}
