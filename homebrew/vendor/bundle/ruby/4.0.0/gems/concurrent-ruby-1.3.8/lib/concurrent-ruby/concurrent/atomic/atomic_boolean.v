...
pub fn atomic_boolean_description(super_description string, value bool) string {
...
fn atomic_boolean_description_from_args(args []brew_runtime.Value) string {
	...
	if args.len == 1 {
		return atomic_boolean_description(args[0].attribute('super') or {
			panic('AtomicBoolean#to_s requires the superclass description')
		}, (args[0].attribute('value') or {
			panic('AtomicBoolean#to_s requires the current value')
		}).bool())
	}
	panic('AtomicBoolean#to_s requires a receiver or superclass description and value')
}
