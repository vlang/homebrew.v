...
pub fn monotonic_time(unit string) !ruby.Value {
	nanoseconds := time.sys_mono_now()
	return match unit.trim_left(':') {
		'', 'float_second' { ruby.float_value(f64(nanoseconds) / 1_000_000_000.0) }
		'float_millisecond' { ruby.float_value(f64(nanoseconds) / 1_000_000.0) }
		'float_microsecond' { ruby.float_value(f64(nanoseconds) / 1_000.0) }
		'second' { ruby.int_value(i64(nanoseconds / 1_000_000_000)) }
		'millisecond' { ruby.int_value(i64(nanoseconds / 1_000_000)) }
		'microsecond' { ruby.int_value(i64(nanoseconds / 1_000)) }
		'nanosecond' { ruby.int_value(i64(nanoseconds)) }
		else { error('unexpected unit: ${unit}') }
	}
}

// Ruby method `monotonic_time(unit = :float_second)` at line 15.
pub fn ruby_monotonic_time_l15_d1_monotonic_time(args ...ruby.Value) ruby.Value {
