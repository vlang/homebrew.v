...
pub fn ruby_hackage_l43_d1_self_match(args ...ruby.Value) ruby.Value {
	if args.len == 0 {
		return ruby.bool_value(false)
	}
	return ruby.bool_value(hackage_matches_url(args[0].as_string()))
}

// Ruby method `self.generate_input_values(url)` at line 55.
pub fn ruby_hackage_l55_d2_self_generate_input_values(args ...ruby.Value) ruby.Value {
	if args.len == 0 {
...
pub fn ruby_hackage_l89_d3_self_find_versions(args ...ruby.Value) ruby.Value {
	if args.len == 0 {
		return ruby.map_value({})
	}
	...
	result := hackage_find_versions(HackageFindRequest{
		...
		content: content
	}, hackage_empty_fetcher) or { return ruby.object_value('Error', err.msg()) }
