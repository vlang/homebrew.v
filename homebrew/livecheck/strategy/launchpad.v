...
pub fn ruby_launchpad_l45_d1_self_match(args ...ruby.Value) ruby.Value {
	if args.len == 0 {
		return ruby.bool_value(false)
	}
	return ruby.bool_value(launchpad_matches_url(args[0].as_string()))
}

// Ruby method `self.generate_input_values(url)` at line 57.
pub fn ruby_launchpad_l57_d2_self_generate_input_values(args ...ruby.Value) ruby.Value {
	if args.len == 0 {
...
pub fn ruby_launchpad_l86_d3_self_find_versions(args ...ruby.Value) ruby.Value {
	if args.len == 0 {
		return ruby.map_value({})
	}
	...
	result := launchpad_find_versions(LaunchpadFindRequest{
		...
		content: content
	}, launchpad_empty_fetcher) or { return ruby.object_value('Error', err.msg()) }
