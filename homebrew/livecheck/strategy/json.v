...
pub fn ruby_json_l45_d1_self_match(url string) bool {
	return json_matches(url)
}

// Ruby method `self.parse_json(content)` at line 52.
pub fn ruby_json_l52_d2_self_parse_json(content string) !json2.Any {
	return json_parse_json(content)
}

// Ruby method `self.versions_from_content(content, regex = nil, &block)` at line 77.
pub fn ruby_json_l77_d3_self_versions_from_content(request JsonVersionsRequest) ![]string {
	return json_versions_from_content(request)
}

// Ruby method `self.find_versions(url:, regex: nil, content: nil, options: Options.new, &block)` at line 108.
pub fn ruby_json_l108_d4_self_find_versions(request JsonFindVersionsRequest,
	fetcher livecheck.StrategyContentFetcher) !JsonMatchData {
	return json_find_versions(request, fetcher)
}
