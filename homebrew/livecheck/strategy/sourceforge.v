...
pub fn ruby_sourceforge_l51_d1_self_match(args ...ruby.Value) ruby.Value {
	if args.len == 0 {
		return ruby.bool_value(false)
	}
	return ruby.bool_value(sourceforge_matches_url(args[0].as_string()))
}

// Ruby method `self.generate_input_values(url)` at line 63.
pub fn ruby_sourceforge_l63_d2_self_generate_input_values(args ...ruby.Value) ruby.Value {
	if args.len == 0 {
	...
	if generated.has_url {
		values['url'] = ruby.string_value(generated.url)
	}
	return ruby.map_value(values)
}

// Ruby method `self.find_versions(url:, regex: nil, content: nil, options: Options.new, &block)` at line 101.
pub fn ruby_sourceforge_l101_d3_self_find_versions(args ...ruby.Value) ruby.Value {
	if args.len == 0 {
		return ruby.map_value({})
	}
	...
	result := sourceforge_find_versions(SourceforgeFindRequest{
		...
		content: content
	}, sourceforge_empty_fetcher) or { return ruby.object_value('Error', err.msg()) }
