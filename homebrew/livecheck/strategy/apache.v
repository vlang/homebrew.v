...
pub fn ruby_apache_l50_d1_self_match(args ...ruby.Value) ruby.Value {
	if args.len == 0 {
		return ruby.bool_value(false)
	}
	return ruby.bool_value(apache_matches_url(args[0].as_string()))
}

// Ruby method `self.generate_input_values(url)` at line 61.
pub fn ruby_apache_l61_d2_self_generate_input_values(args ...ruby.Value) ruby.Value {
	if args.len == 0 {
...
pub fn ruby_apache_l102_d3_self_find_versions(args ...ruby.Value) ruby.Value {
	if args.len == 0 {
		return ruby.map_value({})
	}
	...
	result := apache_find_versions(ApacheFindRequest{
		...
		content: content
	}, apache_empty_fetcher) or { return ruby.object_value('Error', err.msg()) }
