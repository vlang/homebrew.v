...
pub:
	data        map[string]brew_runtime.Value
	output_path string
	contents    string
}
...
fn generate_advisories_api_truthy(value brew_runtime.Value) bool {
...
fn generate_advisories_api_record_id(record map[string]brew_runtime.Value) string {
	return if id := record['id'] { id.as_string() } else { '' }
}

fn generate_advisories_api_sort_records(mut records []map[string]brew_runtime.Value) {
...
fn generate_advisories_api_formula_name(record map[string]brew_runtime.Value, path string) !string {
...
pub fn generate_advisories_api_parse(path string) !map[string]brew_runtime.Value {
	contents := os.read_file(path) or { return error('${path}: ${err.msg()}') }
	value := brew_runtime.parse_json_value(contents) or { return error('${path}: ${err.msg()}') }
	return value.as_map() or { return error('${path}: ${err.msg()}') }
}

pub fn generate_advisories_api_actionable(record map[string]brew_runtime.Value) bool {
	mut source := ''
	...
	mut affected := []brew_runtime.Value{}
...
pub fn generate_advisories_api_build(directory string) !map[string]brew_runtime.Value {
	...
	mut by_formula := map[string][]map[string]brew_runtime.Value{}
	...
	formula_names.sort()
	mut advisories := map[string]brew_runtime.Value{}
	mut count := 0
	for formula_name in formula_names {
		mut records := by_formula[formula_name].clone()
		generate_advisories_api_sort_records(mut records)
		count += records.len
		advisories[formula_name] = brew_runtime.array_value(records.map(brew_runtime.map_value(it)))
	}
	schema_version := if schema_versions.len == 0 {
		brew_runtime.object_value('NilClass', 'nil')
	} else {
		brew_runtime.string_value(schema_versions[0])
	}
	return {
		'meta':       brew_runtime.map_value({
			'count':                brew_runtime.int_value(count)
			'skipped_uncomparable': brew_runtime.int_value(skipped_uncomparable)
			'schema_version':       schema_version
		})
		'advisories': brew_runtime.map_value(advisories)
	}
}
...
pub fn run_generate_advisories_api(options GenerateAdvisoriesApiOptions) !GenerateAdvisoriesApiResult {
	...
	contents := '${brew_runtime.json_value_to_string(brew_runtime.map_value(data))}\n'
...
pub fn generate_advisories_api_input_boundary(input &GenerateAdvisoriesApiInput) brew_runtime.Value {
	return brew_runtime.structured_value('Homebrew::DevCmd::GenerateAdvisoriesApi::Input', '', {
		'generate_advisories_api_input_address': u64(voidptr(input)).str()
	})
}

fn generate_advisories_api_input_from_value(value brew_runtime.Value) !&GenerateAdvisoriesApiInput {
...
fn generate_advisories_api_result_value(result GenerateAdvisoriesApiResult) brew_runtime.Value {
	return brew_runtime.map_value({
		'data':        brew_runtime.map_value(result.data)
		'output_path': brew_runtime.string_value(result.output_path)
...
pub fn ruby_generate_advisories_api_l84_d4_actionable(args ...brew_runtime.Value) brew_runtime.Value {
	if args.len == 0 {
		return brew_runtime.object_value('ArgumentError', 'record is required')
	}
	record := args[0].as_map() or {
		return brew_runtime.object_value('TypeError', err.msg())
	}
	return brew_runtime.bool_value(generate_advisories_api_actionable(record))
}
