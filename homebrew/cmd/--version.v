...
pub fn version_string(repository string, git_executable string) string {
	if !ruby.is_dir(repository) {
		return 'N/A'
	}
	result := ruby.run_command(git_executable, [
		'-C',
		repository,
		'log',
		'-1',
...
pub fn version_lines(config VersionCommandConfig) []string {
	...
	if config.no_install_from_api || ruby.is_dir(config.core_repository) {
		lines << 'Homebrew/homebrew-core ${version_string(config.core_repository, config.git_executable)}'
	}
	if ruby.is_dir(config.cask_repository) {
...
fn configured_homebrew_version(repository string, git_executable string) string {
	configured := ruby.environment_value('HOMEBREW_VERSION')
	if configured != '' {
		return configured
	}
...
pub fn version_lines_from_environment() []string {
	...
	mut git_executable := ruby.environment_value('HOMEBREW_GIT')
	if git_executable == '' {
		git_executable = 'git'
	}
	mut core_repository := ruby.environment_value('HOMEBREW_CORE_REPOSITORY')
	if core_repository == '' {
		core_repository = '${library}/Taps/homebrew/homebrew-core'
	}
	mut cask_repository := ruby.environment_value('HOMEBREW_CASK_REPOSITORY')
	if cask_repository == '' {
	...
	return version_lines(VersionCommandConfig{
		...
		no_install_from_api: ruby.environment_value('HOMEBREW_NO_INSTALL_FROM_API') != ''
		git_executable: git_executable
	})
}

// Ruby method `self.command_name = "--version"` at line 13.
pub fn ruby_version_l13_d1_self_command_name(args ...ruby.Value) ruby.Value {
	_ = args
